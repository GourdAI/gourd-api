...
//
// 挂载位置：每条网关链的 effectiveGroup（生效分组决议）之后、compositeTarget 之前。
// 两个约束同时满足：
//   - 在 effectiveGroup 之后 → 读到的 apiKey.Group 是**生效分组**，白名单按生效分组判（P1④）；
//   - 在 compositeTarget 之前 → 模型仍是客户端书写的公开模型名，未被改写成上游真实模型。
//
// effectiveGroup 只读模型、不改写模型，compositeTarget 才改写模型，因此两者顺序不可互换。
//
// 行为：
...
//
// 未定价模型准入不在本中间件里：见 ModelPricingAdmission（model_pricing_admission.go）。
// 白名单只看客户端书写的公开模型名，必须在合成路由改写之前判定；定价要看改写后
// 的上游模型名与生效分组，必须在改写之后判定。两者挂载位置相反，故不可合并。
//
// 注意：apiKey.Group 为 nil 时直接放行（未绑定分组的 key 由 requireGroupAssignment 拦截）。
