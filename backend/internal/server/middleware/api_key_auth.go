...
// abortIfAPIKeyGroupUnavailable 多分组语义：候选集合中至少一个分组可用则放行。
// 候选全空（全部 deleted/disabled）才拒绝，错误码沿用既有的 GROUP_DELETED / GROUP_DISABLED。
...
func validateAPIKeyGroupAllowed(apiKey *service.APIKey) bool {
	...
	candidates := apiKeyGroupCandidates(apiKey)
	if len(candidates) == 0 {
		// 候选集合为空但存在主分组：退化为单分组判定，避免历史快照（无候选集合）误拦。
		if apiKey.Group == nil {
			return true
		}
		candidates = []*service.Group{apiKey.Group}
	}
...
// validateAPIKeyGroupAvailable 判定 key 是否有「至少一个状态可用的候选分组」。
// 候选全空时分两段报错：
//  1. 全部为 deleted（含分组对象缺失）→ GROUP_DELETED；
//  2. 否则只要存在停用分组 → GROUP_DISABLED。
func validateAPIKeyGroupAvailable(apiKey *service.APIKey) (string, string, bool) {
	...
	candidates := apiKeyGroupCandidates(apiKey)
	if len(candidates) == 0 {
		// 无候选对象：退化单分组判定（主分组对象为空视为已删除，与旧行为一致）。
		group := apiKey.Group
...
// apiKeyGroupCandidates 返回 key 的候选分组对象集合（主分组优先，已去重）。
//
// 快照物化后 apiKey.Groups 已按 GroupIDs 顺序排列；缺失时退化为
// 「主分组」或「GroupIDs 与 Groups 的可用交集」。
func apiKeyGroupCandidates(apiKey *service.APIKey) []*service.Group {
	if apiKey == nil {
		return nil
	}
