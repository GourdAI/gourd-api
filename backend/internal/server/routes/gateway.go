...
func RegisterGatewayRoutes(
	...
	// 请求期「生效分组」决议：必须在 groupModelAllowlist 与 compositeTarget 之前注册。
	// 顺序契约（P1④）：白名单必须按**生效分组**判定，所以 effectiveGroup 先跑；
	// 而 effectiveGroup 只读模型、不改写模型（compositeTarget 才把公开别名改写成上游真实
	// 模型），因此白名单看到的仍是客户端书写的公开模型名——两者都满足，顺序不可再调。
	// 若把白名单放在前面，它会读主分组的 ModelAllowlist：生效分组更严时可被绕过，
	// 主分组更严时会 404 掉生效分组本可服务的合法模型。
	effectiveGroup := effectiveGroupMiddleware(h.Gateway.SchedulingService(), subscriptionService)

	// 未分组 Key 拦截中间件（按协议格式区分错误响应）
	requireGroupAnthropic := middleware.RequireGroupAssignment(settingService, middleware.AnthropicErrorWriter)
	requireGroupGoogle := middleware.RequireGroupAssignment(settingService, middleware.GoogleErrorWriter)

	// 分组级模型白名单准入：在 effectiveGroup 之后、compositeTarget 之前——
	// 按生效分组的白名单判定，且只看客户端书写的公开模型名（未被合成路由改写）。
	groupModelAllowlist := middleware.GroupModelAllowlist()

	// 未定价模型准入：必须挂在 compositeTarget 之后——要读 effectiveGroup 覆写后的
	// 生效分组，以及合成路由改写后的上游真实模型名（那才是被计费的对象）。
	// 开关 pricing.require_priced_models（默认开）。
	modelPricingAdmission := middleware.ModelPricingAdmission(h.Gateway.PricingGate())
	...
	countTokensHandler := func(c *gin.Context) {
		switch getGroupPlatform(c) {
		case service.PlatformOpenAI, service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformMiniMax, service.PlatformOpenCodeGo, service.PlatformWorkbuddy, service.PlatformQoder, service.PlatformTrae:
	...
	gateway.Use(effectiveGroup)
	gateway.Use(groupModelAllowlist)
	gateway.Use(compositeTarget)
	gateway.Use(modelPricingAdmission)
	...
	gemini.Use(effectiveGroup)
	gemini.Use(groupModelAllowlist)
	gemini.Use(compositeGeminiTarget)
	gemini.Use(modelPricingAdmission)
	...
	// 根路径别名共用中间件链：生效分组决议在白名单之前、白名单在 compositeTarget 之前；
	// 定价准入在 compositeTarget 之后（需生效分组与改写后的上游模型名），
	// 避免逐条路由手工维护链导致漏挂。
	rootRoute := func(method, path string, limit gin.HandlerFunc, handler gin.HandlerFunc) {
		r.Handle(method, path, limit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), effectiveGroup, groupModelAllowlist, compositeTarget, modelPricingAdmission, requireGroupAnthropic, handler)
	}
	...
	codexDirect.Use(bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), effectiveGroup, groupModelAllowlist, compositeTarget, modelPricingAdmission, requireGroupAnthropic)
	{
	...
	// P1⑤：本链此前漏挂 effectiveGroup，导致多分组 key 在该入口永远按主分组计费/限流。
	antigravityV1.Use(effectiveGroup)
	antigravityV1.Use(groupModelAllowlist)
	antigravityV1.Use(modelPricingAdmission)
	...
	antigravityV1Beta.Use(effectiveGroup)
	antigravityV1Beta.Use(groupModelAllowlist)
	antigravityV1Beta.Use(modelPricingAdmission)
