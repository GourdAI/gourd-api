...
type PricingConfig struct {
	...
	// RequirePricedModels 为 true 时，网关入口拒绝所有无法解析出定价的模型
	// （404 model_not_found），且模型列表接口不再列出它们。默认 true——定价缺失
	// 的模型过去会静默零成本落账（openai_usage.pricing_missing_record_zero_cost），
	// 等于对下游白嫖。置 false 可回退旧的 fail-open 行为。
	RequirePricedModels bool `mapstructure:"require_priced_models"`
}
...
type GatewayConfig struct {
	...
	// AccountMinRequestIntervalMs: 账号级出站最小请求间隔（毫秒）。
	// 同一账号两次出站请求之间至少间隔该时长（叠加 AccountRequestJitterMs 抖动），
	// 用于把多用户共享订阅账号的请求节奏拉近真实用户的使用模式。0 表示禁用（默认）。
	// 注意：节奏等待会消耗请求的剩余超时预算；配置较大间隔时应确认网关超时足够。
	AccountMinRequestIntervalMs int `mapstructure:"account_min_request_interval_ms"`
	// AccountRequestJitterMs: 账号级出站节奏的随机抖动上限（毫秒）。
	// 每次等待叠加一个 [0, jitter) 的随机增量，避免多账号在同一毫秒刻度上周期性出站。
	AccountRequestJitterMs int `mapstructure:"account_request_jitter_ms"`
	...
	// Workbuddy: WorkBuddy 平台的每日签到与积分查询配置。
	Workbuddy GatewayWorkbuddyConfig `mapstructure:"workbuddy"`
...
// GatewayWorkbuddyConfig WorkBuddy 平台的每日签到与任务三件套配置。
//
// 签到仅对国内版（CN realm）账号有效——国际版无签到体系（对齐参考实现 D4 门控）。
//   - checkin_enabled: 是否启用每日自动签到（默认 true）
//   - checkin_hours: 每日签到触发小时（本地时区 0-23，默认 [9, 21]；21 点为兜底重试，
//     重复签到上游幂等返回「今天已签到」）
//   - activity_enabled: 是否启用对话活跃上报周期任务（默认 true；CN 与 global 账号都上报）
//   - activity_hours: 活跃上报触发小时（本地时区，默认 [10]）
//   - activity_report_count: 每号每次上报条数（默认 5；<=0 归 1，兼容旧行为）
//   - travel_enabled: 是否启用猫猫旅行周期任务（默认 true；仅 CN 账号实际执行）
//   - travel_hours: 旅行巡检触发小时（本地时区，默认 [9, 21]；9 点派出 + 21 点兜底领奖）
//
// 小时列表缺省/空时用默认值；越界值丢弃；去重后为空（全部非法）视为关闭该类任务。
type GatewayWorkbuddyConfig struct {
	CheckinEnabled bool  `mapstructure:"checkin_enabled"`
	CheckinHours   []int `mapstructure:"checkin_hours"`

	ActivityEnabled     bool  `mapstructure:"activity_enabled"`
	ActivityHours       []int `mapstructure:"activity_hours"`
	ActivityReportCount int   `mapstructure:"activity_report_count"`
	TravelEnabled       bool  `mapstructure:"travel_enabled"`
	TravelHours         []int `mapstructure:"travel_hours"`
}

// GatewayQoderConfig Qoder 平台的每日活动 Credits 领取配置。
//
// Qoder 的「每日领取 100 Credits」是 campaigns 活动体系（不是单一签到端点）：
// 先 GET /sash/api/v1/me/campaigns 拿到本轮 CLAIMABLE 的 campaignId，再 POST
// 该 id 的 /claim。上游对同一轮重复领取幂等（replayed=true、不再发钱），所以
// 多实例部署无需分布式锁。
//   - checkin_enabled: 是否启用每日自动领取（默认 true）
//   - checkin_hours: 每日领取触发小时（本地时区 0-23，默认 [10, 21]）。活动每天
//     10:00（UTC+8）开放新一轮，10 点为首发，21 点为兵底重试。
//
// 小时列表缺省/空时用默认值；越界值丢弃；去重后为空（全部非法）视为关闭。
...
func setDefaults() {
	...
	viper.SetDefault("pricing.require_priced_models", true)
	...
	// WorkBuddy 每日签到（仅 CN realm 账号；默认 9 点 + 21 点兜底，可配置关闭）。
	viper.SetDefault("gateway.workbuddy.checkin_enabled", true)
	viper.SetDefault("gateway.workbuddy.checkin_hours", []int{9, 21})
	// WorkBuddy 任务三件套：活跃上报（CN+global 都发，默认 10 点，每号 5 条）
	// 与猫猫旅行（仅 CN，默认 9 点派出 + 21 点兜底领奖）。
	viper.SetDefault("gateway.workbuddy.activity_enabled", true)
	viper.SetDefault("gateway.workbuddy.activity_hours", []int{10})
	viper.SetDefault("gateway.workbuddy.activity_report_count", 5)
	viper.SetDefault("gateway.workbuddy.travel_enabled", true)
	viper.SetDefault("gateway.workbuddy.travel_hours", []int{9, 21})
	...
	// 账号级出站节奏控制（默认禁用；启用后同一账号请求按最小间隔+抖动排开）。
	// 对齐 anti-api 的账号级节流思路，用于降低“同账号高频自动化流量”被风控识别的概率。
	viper.SetDefault("gateway.account_min_request_interval_ms", 0)
	viper.SetDefault("gateway.account_request_jitter_ms", 0)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.AccountMinRequestIntervalMs < 0 || c.Gateway.AccountMinRequestIntervalMs > 600000 {
		return fmt.Errorf("gateway.account_min_request_interval_ms must be between 0-600000 milliseconds")
	}
	if c.Gateway.AccountRequestJitterMs < 0 || c.Gateway.AccountRequestJitterMs > 600000 {
		return fmt.Errorf("gateway.account_request_jitter_ms must be between 0-600000 milliseconds")
	}
