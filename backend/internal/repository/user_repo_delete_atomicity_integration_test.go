...
func TestUserRepository_DeleteUser_AtomicWithAPIKeys(t *testing.T) {
	...
	key1 := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: fmt.Sprintf("tk-atomic-a-%d", user.ID)})
	key2 := mustCreateApiKey(t, client, &service.APIKey{UserID: user.ID, Key: fmt.Sprintf("tk-atomic-b-%d", user.ID)})
