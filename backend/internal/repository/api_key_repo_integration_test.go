...
func (s *APIKeyRepoSuite) TestCreate() {
	...
	s.Require().Equal("tk-create-test", got.Key)
}
...
func (s *APIKeyRepoSuite) TestCreate_AfterSoftDelete_AllowsSameKey() {
	...
	const reusedKey = "tk-reuse-after-soft-delete"
...
func (s *APIKeyRepoSuite) TestExistsByKey() {
	...
	notExists, err := s.repo.ExistsByKey(s.ctx, "tk-not-exists")
	s.Require().NoError(err)
...
func (s *APIKeyRepoSuite) TestSearchAPIKeys() {
	...
	s.mustCreateApiKey(user.ID, "tk-search-1", "Production Key", nil)
	s.mustCreateApiKey(user.ID, "tk-search-2", "Development Key", nil)
...
func (s *APIKeyRepoSuite) TestIncrementQuotaUsed_Basic() {
	...
	key := s.mustCreateApiKey(user.ID, "tk-incr-basic", "Incr", nil)
...
func (s *APIKeyRepoSuite) TestIncrementQuotaUsed_DeletedKey() {
	...
	key := s.mustCreateApiKey(user.ID, "tk-incr-del", "Deleted", nil)
...
func (s *APIKeyRepoSuite) TestIncrementQuotaUsedAndGetState() {
	...
	key := s.mustCreateApiKey(user.ID, "tk-quota-state", "QuotaState", nil)
	key.Quota = 3
	key.QuotaUsed = 1
...
func TestIncrementQuotaUsed_Concurrent(t *testing.T) {
	...
	k := &service.APIKey{
		UserID: u.ID,
		Key:    "tk-concurrent-" + time.Now().Format(time.RFC3339Nano),
		Name:   "Concurrent",
		Status: service.StatusActive,
	}
...
func (s *APIKeyRepoSuite) TestDeleteWithAudit_TombstonesWithoutRetainingCredential() {
	...
	s.Require().NotEqual("tk-del-audit-1", tombstone)
...
func (s *APIKeyRepoSuite) TestDeleteWithAudit_RepeatIsIdempotent() {
	...
	key := &service.APIKey{UserID: user.ID, Key: "tk-del-audit-2", Name: "K", Status: service.StatusActive}
	s.Require().NoError(s.repo.Create(s.ctx, key))
