...
func (r *apiKeyRepository) GetByID(ctx context.Context, id int64) (*service.APIKey, error) {
	...
	if groupIDs, groupErr := r.apiKeyGroupIDsForAuth(ctx, m.ID, m.GroupID); groupErr == nil && len(groupIDs) > 0 {
		out.GroupIDs = groupIDs
	}
	r.attachCandidateGroups(ctx, out)
	return out, nil
}
...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	...
	out := apiKeyEntityToService(m)
	// 候选分组集合：主分组（api_keys.group_id）优先，其余按 sort_order/group_id 升序。
	// 关联表缺失（迁移未执行）时退化为单分组语义，保证向后兼容。
	groupIDs, groupErr := r.apiKeyGroupIDsForAuth(ctx, m.ID, m.GroupID)
	if groupErr != nil {
		return nil, groupErr
	}
	if len(groupIDs) > 0 {
...
func (r *apiKeyRepository) ListByGroupID(ctx context.Context, groupID int64, params pagination.PaginationParams) ([]service.APIKey, *pagination.PaginationResult, error) {
	// 多分组语义：主分组或关联表任一命中即视为绑定该分组。
	// 关联表命中的 key ID 先查出，再与主分组条件取并集（EXISTS 等价语义）。
	boundIDs, err := r.apiKeyIDsByGroupID(ctx, groupID)
	if err != nil {
		return nil, nil, err
	}

	groupFilter := apikey.GroupIDEQ(groupID)
	if len(boundIDs) > 0 {
		groupFilter = apikey.Or(groupFilter, apikey.IDIn(boundIDs...))
	}
	q := r.activeQuery().Where(groupFilter)
...
// ClearGroupIDByGroupID 解除指定分组在所有 API Key 上的绑定（含主分组列与关联表）。
//
// 语义升级：旧实现只清 api_keys.group_id；多分组绑定下必须同时从关联表移除，
// 否则 key 仍会命中「已删除/已下线」分组。
func (r *apiKeyRepository) ClearGroupIDByGroupID(ctx context.Context, groupID int64) (int64, error) {
	// 关联表清理由 UnbindGroupFromAllAPIKeys 完成（它同时负责主分组回填）；
	// 该方法保留原生单值列清理以保证向后兼容（例如测试桩、关联表缺失场景）。
	if r.sql != nil {
		if _, err := r.RemoveGroupFromAllAPIKeys(ctx, groupID); err != nil {
			return 0, err
		}
	}
...
// UnbindGroupFromAllAPIKeys 是 ClearGroupIDByGroupID 的显式命名别名，
// 表达「从所有 key 的绑定集合中移除该分组」的完整语义。
func (r *apiKeyRepository) UnbindGroupFromAllAPIKeys(ctx context.Context, groupID int64) (int64, error) {
	return r.ClearGroupIDByGroupID(ctx, groupID)
}

// UpdateGroupIDByUserAndGroup 将用户下绑定 oldGroupID 的所有 Key 迁移到 newGroupID。
// 多分组语义：关联表与主分组列一并迁移；关联表未命中时仍退化为单值列更新。
func (r *apiKeyRepository) UpdateGroupIDByUserAndGroup(ctx context.Context, userID, oldGroupID, newGroupID int64) (int64, error) {
	if r.sql != nil {
		if _, err := r.MigrateAPIKeyGroupsOnUserGroupChange(ctx, userID, oldGroupID, newGroupID); err != nil {
			return 0, err
		}
	}
	client := clientFromContext(ctx, r.client)
...
// CountByGroupID 获取分组的 API Key 数量（主分组或关联表任一命中）。
func (r *apiKeyRepository) CountByGroupID(ctx context.Context, groupID int64) (int64, error) {
	if r.sql != nil {
		count, err := r.CountAPIKeysByAnyGroupID(ctx, groupID)
		if err == nil && count > 0 {
			return count, nil
		}
		if err != nil {
...
// ListKeysByGroupID 列出某分组下所有 key 明文（主分组或关联表任一命中）。
func (r *apiKeyRepository) ListKeysByGroupID(ctx context.Context, groupID int64) ([]string, error) {
	if r.sql != nil {
		keys, err := r.ListAPIKeyStringsByAnyGroupID(ctx, groupID)
		if err != nil {
			return nil, err
		}
		if len(keys) > 0 {
