...
func (c *geminiOAuthClient) ExchangeCode(ctx context.Context, oauthType, code, codeVerifier, redirectURI, proxyURL string) (*geminicli.TokenResponse, error) {
	...
	// Token 端点伪装：官方 CLI 经 google-auth-library 发起该请求，
	// UA 带 google-api-nodejs-client 产品后缀，x-goog-api-client 为 gl-node 指纹。
	req := client.R().
		SetContext(ctx).
		SetFormDataFromValues(formData).
		SetSuccessResult(&tokenResp).
		SetHeader("User-Agent", geminicli.GoogleAPINodeClientSuffix).
		SetHeader(geminicli.CodeAssistAPIClientHeader, geminicli.CodeAssistNodeClientValue)
