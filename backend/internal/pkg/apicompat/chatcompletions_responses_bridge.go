...
func normalizeChatMessagesWithToolOutputMedia(messages []ChatMessage, mediaByCallID toolOutputMediaByCallID) []ChatMessage {
	...
	for _, m := range messages {
		...
		case len(m.ToolCalls) > 0:
			...
			seenCall := make(map[string]bool, len(m.ToolCalls))
			for _, tc := range m.ToolCalls {
				if tc.ID == "" {
					continue
				}
				// 同一条 assistant 内重复的 tool_call id（旧版网关 completed 换发
				// item id 污染过的客户端历史）只保留一次：上游对重复 id 必拒
				// （11148），去重后调用与结果恢复一一对应。
				if seenCall[tc.ID] {
					continue
				}
...
func (state *ChatCompletionsToResponsesStreamState) chatOutput() []ResponsesOutput {
	...
	if state.Reasoning.Len() > 0 {
		outputs = append(outputs, ResponsesOutput{
			Type: "reasoning",
			// 必须复用流内已宣告的 item id：客户端以「流事件 + response.completed」
			// 合并会话历史并按 item id 去重，completed 里换发新 id 会让同一
			// function_call 被存两份（call_id 相同），下一轮回放即触发上游
			// 11148 tool_call_sequence_broken（同 assistant 内重复 tool_call id）。
