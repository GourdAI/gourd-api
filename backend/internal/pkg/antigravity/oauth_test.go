...
func TestConstants_值正确(t *testing.T) {
	...
	// 不再断言字面量，避免把内置 partner secret 明文写进仓库；
	// 只校验默认值存在且符合 Google OAuth client secret 的格式。
	if !strings.HasPrefix(secret, "GOCSPX-") || len(secret) <= len("GOCSPX-") {
		t.Errorf("默认 client_secret 格式不正确: got %q", secret)
	}
