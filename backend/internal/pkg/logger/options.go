...
func resolveLogFilePath(explicit string) string {
	...
	// Windows 上 "/app/data/logs/sub2api.log" 会被解析为「当前盘符:\app\data\logs\
	// sub2api.log」（如 D:\app\data\logs\sub2api.log）：首次写日志会凭空创建
	// D:\app\data 目录，而 setup.GetDataDir 的 Docker 探测会因该目录存在而把本地
	// 开发环境误判为容器，导致 config.yaml/.installed 不可见、进入 setup 向导。
	// 本地开发默认写到可执行文件同级目录，与 GetDataDir 的 "." 回退保持同一根。
