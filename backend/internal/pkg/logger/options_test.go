...
// defaultLogPathForOS 返回当前平台下无 DATA_DIR 时的默认日志路径。
// Windows 上不能用 /app/data（会被解析为当前盘符下的 \app\data，凭空创建
// D:\app\data 目录并让 setup.GetDataDir 误判为容器环境），故走本地相对目录。
func defaultLogPathForOS() string {
	if runtime.GOOS == "windows" {
		return filepath.Join("data", "logs", "sub2api.log")
	}
...
func TestResolveLogFilePath_Default(t *testing.T) {
	...
	want := defaultLogPathForOS()
	if got != want {
		t.Fatalf("resolveLogFilePath() = %q, want %q", got, want)
	}
	if runtime.GOOS == "windows" && strings.HasPrefix(got, "/") {
		t.Fatalf("windows default log path must not be absolute POSIX: %q", got)
	}
}
...
func TestNormalizedOptions_InvalidFallback(t *testing.T) {
	...
	if out.Output.FilePath != defaultLogPathForOS() {
