...
type ChannelService struct {
	...
	// 未定价模型不上架「可用渠道」（与网关入口拒绝同口径）；nil 时不过滤（测试场景）。
	// 由 wire 在 resolver 构造后经 SetModelPricingGate 注入（构造顺序原因）。
	modelPricingGate *ModelPricingGate
...
// SetModelPricingGate 注入定价闸门（供「可用渠道」展示过滤未定价模型）。
// 单独 setter 而不是构造参数：ChannelService 构造早于 ModelPricingResolver。
func (s *ChannelService) SetModelPricingGate(gate *ModelPricingGate) {
	if s == nil {
		return
	}
	s.modelPricingGate = gate
}
...
func matchingPlatforms(groupPlatform string) []string {
	if groupPlatform == PlatformComposite {
		return []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo, PlatformWorkbuddy, PlatformQoder, PlatformTrae}
	}
...
// HasChannelModelPricing 报告「分组+模型」是否存在带实际价格的渠道定价卡。
//
// 与 GetChannelModelPricing 走完全相同的查找路径（精确 → 通配，平台维度一致），
// 区别只在于不 Clone 价卡：定价准入与模型列表过滤只需要知道「有没有价」，而列表
// 过滤会对数百个模型逐个调用，Clone 的深拷贝（含 intervals 切片）是纯浪费。
//
// 空价卡（只绑了模型名、所有价格字段为空）按「无价」处理，与 pricingNeedsFallback
// 在计费链里的语义一致——那种卡等于免费放行，不能算已定价。
func (s *ChannelService) HasChannelModelPricing(ctx context.Context, groupID int64, model string) bool {
	lk, err := s.lookupGroupChannel(ctx, groupID)
	if err != nil {
		slog.Warn("failed to load channel cache", "group_id", groupID, "error", err)
		return false
	...
	pricing := lookupPricingAcrossPlatforms(lk.cache, groupID, lk.platform, strings.ToLower(model))
	return !pricingNeedsFallback(pricing)
}
