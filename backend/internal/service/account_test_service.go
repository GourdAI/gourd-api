...
type AccountTestService struct {
	...
	// qoderUserinfoFetch 是 Qoder 测试路径身份自愈的 userinfo 拉取函数
	// （可选注入：生产 nil 回落官方实现；单测注入 stub）。
	qoderUserinfoFetch qoderUserinfoFetcher
}

// resolveTLSProfileForAccount 返回账号的 TLS Profile；服务未注入时返回 nil（降级为普通 Do）。
func (s *AccountTestService) resolveTLSProfileForAccount(account *Account) *tlsfingerprint.Profile {
...
func (s *AccountTestService) testClaudeAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	resp, err := s.httpUpstream.DoWithTLS(req, proxyURL, account.ID, account.Concurrency, s.resolveTLSProfileForAccount(account))
	if err != nil {
...
func (s *AccountTestService) testGrokResponsesConnection(c *gin.Context, ctx context.Context, account *Account, authToken, testModelID string) error {
	...
	resp, err := doUpstreamWithOptionalTLS(s.httpUpstream, s.tlsFPProfileService, req, s.grokTestProxyURL(account), account)
	if err != nil {
...
func (s *AccountTestService) testGrokImageGeneration(c *gin.Context, ctx context.Context, account *Account, authToken, modelID, prompt, imageDataURL string) error {
	...
	for attempt := 0; attempt < 2; attempt++ {
		...
		resp, doErr = doUpstreamWithOptionalTLS(s.httpUpstream, s.tlsFPProfileService, req, s.grokTestProxyURL(account), account)
		if doErr == nil {
			break
		}
...
func (s *AccountTestService) testGrokVideoGeneration(c *gin.Context, ctx context.Context, account *Account, authToken, modelID, prompt string, opts AccountTestOptions) error {
	...
	for time.Now().Before(deadline) {
		...
		statusResp, err := doUpstreamWithOptionalTLS(s.httpUpstream, s.tlsFPProfileService, statusReq, s.grokTestProxyURL(account), account)
		if err != nil {
...
func (s *AccountTestService) buildGeminiAPIKeyRequest(ctx context.Context, account *Account, modelID string, payload []byte) (*http.Request, error) {
	...
	req.Header.Set("User-Agent", geminicli.CLIUserAgent(modelID))
	geminicli.ApplyAIStudioFingerprintHeaders(req.Header)
	if uid := geminicli.InstallationIDForAccount(account.ID); uid != "" {
		req.Header.Set("x-gemini-api-privileged-user-id", uid)
	}
...
func (s *AccountTestService) buildCodeAssistRequest(ctx context.Context, accessToken, projectID, modelID string, payload []byte) (*http.Request, error) {
	...
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+accessToken)
	req.Header.Set("User-Agent", geminicli.CLIUserAgentWithAuthLibrary(modelID))
	geminicli.ApplyCodeAssistFingerprintHeaders(req.Header)
