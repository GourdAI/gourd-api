...
type RateLimitService struct {
	...
	// rateLimit429CounterCache 追踪 429 兜底事件的连续次数，用于分级冷却。
	rateLimit429CounterCache RateLimit429CounterCache
...
// SetRateLimit429CounterCache 设置 429 兜底连续事件计数器（可选依赖）。
// 未注入时分级冷却自动退化为原固定回避行为。
func (s *RateLimitService) SetRateLimit429CounterCache(cache RateLimit429CounterCache) {
	s.rateLimit429CounterCache = cache
}
...
func (s *RateLimitService) apply429FallbackRateLimit(ctx context.Context, account *Account, reason string) {
	...
	// 分级冷却：连续 429 兜底事件按次数递增回避时长（参照 anti-api 的
	// 60s → 5min → 30min → 2h 阶梯），避免被拒绝后立即重试的机器行为。
	// 第 1 次保持配置的默认回避（容忍瞬时抖动）；计数器未注入时跳过升级。
	if escalated := s.escalate429FallbackCooldown(ctx, account, cooldown); escalated > cooldown {
		slog.Warn("rate_limit_429_fallback_escalated", "account_id", account.ID, "platform", account.Platform, "reason", reason, "cooldown", escalated.String())
		cooldown = escalated
	}
...
// escalate429FallbackCooldown 递增 429 兜底计数并返回分级回避时长。
// 计数失败或未配置计数器时返回原值（不升级）。
func (s *RateLimitService) escalate429FallbackCooldown(ctx context.Context, account *Account, base time.Duration) time.Duration {
	if s == nil || s.rateLimit429CounterCache == nil || account == nil || account.ID <= 0 {
		return base
	}
	count, err := s.rateLimit429CounterCache.IncrementRateLimit429Count(ctx, account.ID, rateLimit429CounterWindowSeconds)
	if err != nil {
		slog.Warn("rate_limit_429_counter_increment_failed", "account_id", account.ID, "error", err)
		return base
	}
	if escalated := rateLimit429EscalationCooldown(count); escalated > 0 {
		return escalated
	}
	return base
}

// ResetRateLimit429Counter 成功后清零 429 兜底计数器（供各平台成功路径调用）。
func (s *RateLimitService) ResetRateLimit429Counter(ctx context.Context, accountID int64) {
	if s == nil || s.rateLimit429CounterCache == nil || accountID <= 0 {
		return
	}
	if err := s.rateLimit429CounterCache.ResetRateLimit429Count(ctx, accountID); err != nil {
		slog.Warn("rate_limit_429_counter_reset_failed", "account_id", accountID, "error", err)
	}
}
...
func (s *RateLimitService) ClearRateLimit(ctx context.Context, accountID int64) error {
	...
	s.ResetRateLimit429Counter(ctx, accountID)
