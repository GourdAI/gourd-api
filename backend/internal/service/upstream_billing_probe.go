...
func IsUpstreamBillingProbeIdentity(platform, accountType string) bool {
	...
	// WorkBuddy 不在名单内：其凭据为 access/refresh token（无 credentials.api_key），
	// 探测取数必然 missing_api_key 失败，故平台层面不具备探测资格——调度器、
	// 创建/更新入口与手动探测会一并拒绝，存量 enabled=true 的账号也会被跳过。
...
// WorkBuddy official domains (copilot.tencent.com / workbuddy.ai) are the same.
// Qoder official domains (qoder.sh / qoder.com / qoder.com.cn / center.qoder.sh)
// are the same: official APIs that can never host /v1/sub2api/billing.
