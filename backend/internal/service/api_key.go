...
type APIKey struct {
	...
	Name   string
	GroupID *int64
	// GroupIDs 是该 Key 的候选分组集合（主分组优先）。
	// 单分组 Key：与 GroupID 等价的单元素切片；未绑定分组：nil。
	// 请求期「生效分组」决议后，GroupID/Group 会被就地覆写为生效分组，
	// 但 GroupIDs 保持不变（保留候选集合语义，供决议与 /v1/models 并集使用）。
	GroupIDs []int64
	Status   string
	IPWhitelist []string
	IPBlacklist []string
	...
	User                *User
	Group               *Group
	// Groups 是候选分组集合对应的完整对象（按 GroupIDs 同序，主分组优先）。
	// 仅认证快照物化时填充；主分组对象仍写入 Group 字段（下游代码零改动）。
	Groups              []*Group
