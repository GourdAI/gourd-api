...
const (
	...
	// WorkBuddy 双域固定协议平台：CN 为 copilot.tencent.com，国际为 www.workbuddy.ai；
	// 上游聊天路径为 {base}/v2/chat/completions（无 /v1 前缀）。
	DefaultWorkbuddyBaseURL       = "https://copilot.tencent.com"
	DefaultWorkbuddyGlobalBaseURL = "https://www.workbuddy.ai"
	// WorkBuddy billing 域（积分查询 / 每日签到）：CN 为 www.codebuddy.cn
	// （与 chat 域 copilot.tencent.com 分离），国际与 chat 域同为主机 www.workbuddy.ai。
	DefaultWorkbuddyBillingBaseURL = "https://www.codebuddy.cn"
	// Qoder 双域固定协议平台：国际为 api1.qoder.sh，CN 为 gateway.qoder.com.cn；
...
// IsWorkbuddy 报告 platform 是否为 WorkBuddy 平台。
func IsWorkbuddy(platform string) bool {
	return platform == PlatformWorkbuddy
}

// IsQoder 报告 platform 是否为 Qoder 平台（包级函数，供无账号上下文的调用方使用）。
func IsQoder(platform string) bool {
