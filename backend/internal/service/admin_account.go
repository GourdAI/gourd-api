...
func (s *adminServiceImpl) DuplicateAccount(ctx context.Context, id int64, actorScope, operationKey string) (*Account, error) {
	...
	if input.Platform == PlatformWorkbuddy {
		if err := NormalizeWorkbuddyCredentials(input.Credentials); err != nil {
			return nil, err
		}
	}
	if input.Platform == PlatformQoder {
		if err := NormalizeQoderCredentials(input.Credentials); err != nil {
			return nil, err
		}
	}
	if input.Platform == PlatformTrae {
		if err := NormalizeTraeCredentials(input.Credentials); err != nil {
			return nil, err
		}
	}
...
func (s *adminServiceImpl) UpdateAccount(ctx context.Context, id int64, input *UpdateAccountInput) (*Account, error) {
	...
	if account.IsCredentialShadow() && input.Credentials != nil {
		...
		if account.Platform == PlatformWorkbuddy {
			if err := NormalizeWorkbuddyCredentials(account.Credentials); err != nil {
				return nil, err
			}
		}
		if account.Platform == PlatformQoder {
			if err := NormalizeQoderCredentials(account.Credentials); err != nil {
				return nil, err
			}
		}
		if account.Platform == PlatformTrae {
			if err := NormalizeTraeCredentials(account.Credentials); err != nil {
				return nil, err
			}
		}
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	// 批量路径平台混合：仅当凭据携带 workbuddy/qoder/trae 专属键时归一化校验（函数自身对
	// 无对应平台键的凭据为 no-op）。
	if err := NormalizeWorkbuddyCredentials(input.Credentials); err != nil {
		return nil, err
	}
