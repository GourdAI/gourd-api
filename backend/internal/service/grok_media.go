...
func (s *OpenAIGatewayService) forwardGrokMediaVideoContent(
	...
	upstreamStart := time.Now()
	// 防封加固：视频 status 轮询与主链路共用同一出站入口，避免指纹旁路。
	statusResp, err := doUpstreamWithOptionalTLS(s.httpUpstream, s.tlsFPProfileService, statusReq, proxyURL, account)
	if err != nil {
		SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
	}
	...
	// 防封加固：视频 content 下载与主链路共用同一出站入口，避免指纹旁路。
	contentResp, err := doUpstreamWithOptionalTLS(s.httpUpstream, s.tlsFPProfileService, contentReq, proxyURL, account)
	SetOpsLatencyMs(c, OpsUpstreamLatencyMsKey, time.Since(upstreamStart).Milliseconds())
	if err != nil {
		return nil, s.handleOpenAIUpstreamTransportError(ctx, c, account, err, false)
	}
