...
func TestUpstreamBillingProbeSuccessPersistsSanitizedSnapshot(t *testing.T) {
	...
	require.Equal(t, "Bearer tk-sensitive", upstream.lastReq.Header.Get("Authorization"))
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			account := &Account{
				...
				Status:      StatusActive,
				Concurrency: 1,
				Credentials: map[string]any{"api_key": "tk-sensitive", "base_url": "https://upstream.example"},
				ProxyID:     &proxyID,
				Proxy:       tc.proxy,
			}
			repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{account.ID: account}}
