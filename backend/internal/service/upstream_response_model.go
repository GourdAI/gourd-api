...
// upstreamModelMismatchForAccount 在 upstreamModelMismatch 基础上叠加平台级豁免。
//
// Qoder：上游 chat 端点对任何模型 key 的响应都恒报 model:"auto"（协议占位值；
// 实测发 qfmodel/auto/qwen3.8-flash 均回 auto），因此「发送 key 为官方 key 且
// 响应为 auto」不是真的模型不一致，属系统性误报，审计口径按一致处理
// （响应模型原文仍照常记录供观测）。发送非官方 key 时不豁免，历史坏配置
// 仍可被审计发现。
func upstreamModelMismatchForAccount(account *Account, sentModel, responseModel string) *bool {
	if account != nil && account.IsQoderPlatform() && isQoderAutoResponseModel(responseModel) {
		sent := strings.ToLower(strings.TrimSpace(sentModel))
		if isQoderModelCode(sent) {
			match := false
			return &match
		}
	}
