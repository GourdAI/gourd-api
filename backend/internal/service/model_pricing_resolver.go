...
// hasChannelPricingNormalized 是 lookupChannelPricingNormalized 的「只问有没有价」变体：
// 同样先按字面模型名查、未命中再按 OpenAI/Codex 归一化基名查一次，但不 Clone 价卡。
// 供定价准入闸门在热路径与列表过滤上使用。
func (r *ModelPricingResolver) hasChannelPricingNormalized(ctx context.Context, groupID int64, model string) bool {
	if r == nil || r.channelService == nil {
		return false
	}
	if r.channelService.HasChannelModelPricing(ctx, groupID, model) {
		return true
	}
