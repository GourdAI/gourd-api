...
type antigravityRetryLoopParams struct {
	...
	// tlsProfile 为启用了 TLS 指纹的账号解析出的出站 Profile（nil 表示不启用，防封加固）。
	tlsProfile *tlsfingerprint.Profile
}

// doUpstream 按 tlsProfile 选择 Do/DoWithTLS（防封加固统一出站入口）。
func (p antigravityRetryLoopParams) doUpstream(req *http.Request) (*http.Response, error) {
	if p.tlsProfile != nil {
		return p.httpUpstream.DoWithTLS(req, p.proxyURL, p.account.ID, p.account.Concurrency, p.tlsProfile)
	}
...
func (s *AntigravityGatewayService) handleSmartRetry(p antigravityRetryLoopParams, resp *http.Response, respBody []byte, baseURL string, urlIdx int, availableURLs []string) *smartRetryResult {
	...
	if shouldSmartRetry {
		...
		for attempt := 1; attempt <= maxAttempts; attempt++ {
			...
			retryResp, retryErr := p.doUpstream(retryReq)
