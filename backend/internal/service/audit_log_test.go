...
func TestMaskAuditCredential(t *testing.T) {
	cases := []struct {
		...
		{"long", "Zx-qqq-zzz07-abcdefghijklmnop1234", "Zx-qqq****1234"},
	}
...
func TestRedactAuditBody_JSONRedactsSecrets(t *testing.T) {
	raw := []byte(`{
		...
		"credentials": {"api_key": "tk-secret-123", "base_url": "https://evil.example.com"},
	...
	// 敏感字段被擦除。
	for _, secret := range []string{"tk-secret-123", "hunter2", "123456", "tok_abc"} {
...
func TestRedactAuditBody_AuthoritativeTablesSynced(t *testing.T) {
	...
	for _, secret := range []string{
		"tk-session-aaa", "pem-body-bbb", "sa-blob-ccc",
		"proxypass-ddd", "tk-custom-eee",
