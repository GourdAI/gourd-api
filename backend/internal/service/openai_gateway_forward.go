...
func shouldForwardOpenAIResponsesViaRawChatCompletions(account *Account) bool {
	...
	if account.IsWorkbuddy() {
		// WorkBuddy 上游只提供 Chat Completions 形态的 /v2/chat/completions（无
		// /v1/responses）：三条入站路径（CC / Messages / Responses）统一路由到
		// 各自的 raw-CC 回退函数，由 workbuddy 专用发送管线出站。
		return true
	}
	if account.IsQoderPlatform() {
		// Qoder 上游只提供 COSY 协议的 chat SSE 端点（agent_chat_generation，无
		// /v1/responses）：与 WorkBuddy 同语义，三条入站路径统一走 raw-CC 回退，
		// 由 qoder 专用发送管线出站。
		return true
	}
