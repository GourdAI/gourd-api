...
type GeminiMessagesCompatService struct {
	...
	// tlsFPProfileService 为启用了 TLS 指纹的账号提供出站 Profile（可选依赖，防封加固）。
	tlsFPProfileService *TLSFingerprintProfileService
}

// SetTLSFingerprintProfileService 注入 TLS 指纹模板服务（可选依赖）。
func (s *GeminiMessagesCompatService) SetTLSFingerprintProfileService(svc *TLSFingerprintProfileService) {
...
func (s *GeminiMessagesCompatService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
	...
	case AccountTypeAPIKey:
		buildReq = func(ctx context.Context) (*http.Request, string, error) {
			...
			upstreamReq.Header.Set("User-Agent", geminicli.CLIUserAgent(mappedModel))
			geminicli.ApplyAIStudioFingerprintHeaders(upstreamReq.Header)
			if uid := geminicli.InstallationIDForAccount(account.ID); uid != "" {
				upstreamReq.Header.Set("x-gemini-api-privileged-user-id", uid)
			}
	...
	case AccountTypeOAuth:
		buildReq = func(ctx context.Context) (*http.Request, string, error) {
			...
			if projectID != "" {
				...
				upstreamReq.Header.Set("User-Agent", geminicli.CLIUserAgent(mappedModel))
				geminicli.ApplyCodeAssistFingerprintHeaders(upstreamReq.Header)
				return upstreamReq, "x-request-id", nil
			} else {
	...
	for attempt := 1; attempt <= geminiMaxRetries; attempt++ {
		...
		resp, err = doUpstreamWithOptionalTLS(s.httpUpstream, s.tlsFPProfileService, upstreamReq, proxyURL, account)
		if err != nil {
...
func (s *GeminiMessagesCompatService) ForwardAIStudioGET(ctx context.Context, account *Account, path string) (*UpstreamHTTPResult, error) {
	...
	resp, err := doUpstreamWithOptionalTLS(s.httpUpstream, s.tlsFPProfileService, req, proxyURL, account)
	if err != nil {
		return nil, err
	}
	defer func() { _ = resp.Body.Close() }()
