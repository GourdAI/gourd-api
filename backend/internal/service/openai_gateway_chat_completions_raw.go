...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	// WorkBuddy 账号走专用 CC 入站转发：workbuddy 协议变换、专用头与 SSE 归一
	// 均在 sendWorkbuddyUpstreamRequest 内完成，不走通用 CC 管线。
	if account.IsWorkbuddy() {
		return s.forwardWorkbuddyChatCompletions(ctx, c, account, body, defaultMappedModel)
	}
	// Qoder 账号同语义：COSY 协议变换、签名头与 SSE 归一均在 sendQoderUpstreamRequest
	// 内完成，不走通用 CC 管线。
