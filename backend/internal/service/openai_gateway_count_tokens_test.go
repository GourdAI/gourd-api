...
// WorkBuddy 上游只有 /v2/chat/completions（无 count_tokens 端点）：与
// CN 供应商/OpenCode 同语义本地估算，绝不发上游请求。
func TestOpenAIGatewayService_ForwardCountTokensAsAnthropic_WorkbuddyEstimatesLocally(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"object":"response.input_tokens","input_tokens":999}`)),
	}}
	svc := &OpenAIGatewayService{
		cfg: &config.Config{Security: config.SecurityConfig{URLAllowlist: config.URLAllowlistConfig{
			Enabled:           false,
	...
	account := &Account{
		ID:          303,
		Name:        "workbuddy-apikey",
		Platform:    PlatformWorkbuddy,
		Type:        AccountTypeAPIKey,
		Concurrency: 1,
		Credentials: map[string]any{
			"access_token": "tok-303",
			"base_url":     "http://upstream.example",
		},
		Status:      StatusActive,
		Schedulable: true,
	...
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, rec.Code)
	require.Greater(t, gjson.GetBytes(rec.Body.Bytes(), "input_tokens").Int(), int64(0), "本地估算返回正整数")
	require.NotEqual(t, int64(999), gjson.GetBytes(rec.Body.Bytes(), "input_tokens").Int(), "响应来自本地估算而非上游")
	require.Nil(t, upstream.lastReq, "workbuddy 不发上游请求")
}
