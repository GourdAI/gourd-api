...
type CreateAPIKeyRequest struct {
	...
	// GroupIDs 多分组绑定；nil=未提供（按 GroupID 单分组处理），空数组=不绑定任何分组。
...
type UpdateAPIKeyRequest struct {
	Name    *string `json:"name"`
	GroupID *int64  `json:"group_id"`
	// GroupIDs 整体替换绑定集合（nil=不修改，空数组=解绑全部）；同时传时优先于 GroupID。
	GroupIDs    *[]int64  `json:"group_ids"`
...
//
// 冻结契约：
//   - group_ids 优先于 group_id；
//   - 只传 group_id 等价于单元素列表；
//   - group_ids 为空数组 → 解绑全部（返回空集合与 nil 主分组）；
//   - 主分组 = 集合第一个元素（写入 api_keys.group_id），**顺序即语义，不得重排**。
//
// 顺序契约（重要）：前端按用户勾选顺序提交 group_ids，并以 group_ids[0] 作为主分组
// （见 frontend/src/api/keys.ts：payload.group_id = groupIds[0]，UI 的「主分组」徽章
// 也标注 modelValue[0]）。因此归一化必须保持传入顺序，主分组取 normalized[0]，
// 绝不可按 ID 升序重排——否则用户选定的主分组会被静默换成 ID 最小的分组，
// 导致 GET /v1/models 等「模型不可知」请求回退到错误分组，计费倍率/组级 RPM/
// usage_logs.group_id 全部落错。
//
// 返回 (groupIDs, primaryGroupID, provided)：provided=false 表示本次请求未涉分组修改。
func resolveRequestedGroupIDs(groupID *int64, groupIDs *[]int64) ([]int64, *int64, bool) {
	if groupIDs != nil {
		normalized := normalizeAPIKeyGroupIDsForService(*groupIDs)
		if len(normalized) == 0 {
			return nil, nil, true
		}
		// 主分组 = 用户勾选顺序的第一个（normalized 已保序）。
		primary := normalized[0]
		return normalized, &primary, true
	}
	if groupID != nil {
		if *groupID <= 0 {
			return nil, nil, true
		...
		primary := *groupID
		return []int64{primary}, &primary, true
	}
	return nil, nil, false
}

// normalizeAPIKeyGroupIDsForService 去重 + 去除非正值，**保持传入顺序**（首个元素即主分组）。
// 顺序是语义的一部分：前端按用户勾选顺序提交，group_ids[0] 为主分组，不得重排。
func normalizeAPIKeyGroupIDsForService(groupIDs []int64) []int64 {
...
func (s *APIKeyService) validateBindableGroups(ctx context.Context, user *User, groupIDs []int64) error {
	...
	for _, groupID := range groupIDs {
		...
		// 状态校验：与 admin 侧同口径，禁止把已停用分组写进绑定集合（P3）。
		// 软删除分组已被 GetByID 拦为 not found，这里只需挡 inactive。
		if group.Status != StatusActive {
			return infraerrors.BadRequest("GROUP_NOT_ACTIVE", fmt.Sprintf("group %d is not active", groupID))
		}
		if !s.canUserBindGroup(ctx, user, group) {
			return fmt.Errorf("%w: group_id=%d", ErrGroupNotAllowed, groupID)
		}
		// 订阅类型一致性：以第一个分组为基准，后续分组必须同类型。
		...
		if isSub != firstGroupIsSub {
			return infraerrors.BadRequest("MIXED_GROUP_SUBSCRIPTION_TYPE",
...
// replaceAPIKeyGroupsIfSupported 尝试调用仓储的关联表整体替换能力（窄接口断言，
// 不扩大 APIKeyRepository 接口面，保持向后兼容）。
func (s *APIKeyService) replaceAPIKeyGroupsIfSupported(ctx context.Context, apiKeyID int64, groupIDs []int64, primaryGroupID *int64) error {
...
func (s *APIKeyService) Create(ctx context.Context, userID int64, req CreateAPIKeyRequest) (*APIKey, error) {
	...
	// 归一化分组绑定（group_ids 优先于 group_id；主分组 = 集合第一个）。
	requestedGroupIDs, primaryGroupID, groupsProvided := resolveRequestedGroupIDs(req.GroupID, req.GroupIDs)

	// 验证分组权限（集合内每个分组都必须通过 canUserBindGroup 等价校验）
	if groupsProvided && len(requestedGroupIDs) > 0 {
		if err := s.validateBindableGroups(ctx, user, requestedGroupIDs); err != nil {
			return nil, err
		}
	}
	...
	// 写入关联表（多分组候选集合）；单分组时也写，保持两处口径一致。
	if groupsProvided {
		if err := s.replaceAPIKeyGroupsIfSupported(ctx, apiKey.ID, requestedGroupIDs, primaryGroupID); err != nil {
			return nil, fmt.Errorf("create api key groups: %w", err)
		}
	}
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	// 分组绑定：group_ids 优先于 group_id；两者未传时不修改绑定。
	requestedGroupIDs, primaryGroupID, groupsProvided := resolveRequestedGroupIDs(req.GroupID, req.GroupIDs)
	groupsChanged := false
	if groupsProvided {
	...
	// 关联表整体替换（与 api_keys.group_id 写入同一语义：集合整体替换）。
	if groupsChanged {
		if err := s.replaceAPIKeyGroupsIfSupported(ctx, apiKey.ID, requestedGroupIDs, primaryGroupID); err != nil {
