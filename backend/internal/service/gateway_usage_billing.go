...
func (s *GatewayService) RecordUsage(ctx context.Context, input *RecordUsageInput) error {
	// 成功请求清零 429 兜底分级计数器（与 anti-api 的成功恢复语义对齐）。
	if s.rateLimitService != nil && input != nil && input.Account != nil {
		s.rateLimitService.ResetRateLimit429Counter(ctx, input.Account.ID)
	}
