...
//
// 原实现仅适用于 Anthropic OAuth/SetupToken；防封加固后扩展到所有支持 TLS 指纹的
// 订阅类平台（openai/gemini/grok/antigravity/workbuddy/opencode 等），
// 以便统一在出站链路上模拟对应官方客户端的 TLS 握手特征。
//
// 保持账号级 opt-in 语义：默认关闭，显式在 Extra 中设置 enable_tls_fingerprint=true 才启用。
// Anthropic OAuth/SetupToken 账号行为与之前完全一致（该路径本来就是唯一可用组合）。
