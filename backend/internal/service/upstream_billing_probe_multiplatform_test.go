...
// WorkBuddy 凭据为 access/refresh token（无 credentials.api_key），探测取数必然
// missing_api_key 失败，因此平台层面不具备探测资格：调度跳过、创建/更新入口拒绝、
// 存量 enabled=true 的账号也不再被探测。
// Qoder 同理：凭据为设备令牌/会话令牌（无 credentials.api_key），不具备探测资格。
func TestUpstreamBillingProbeIdentityExcludesWorkbuddy(t *testing.T) {
	require.False(t, IsUpstreamBillingProbeIdentity(PlatformWorkbuddy, AccountTypeAPIKey))
	require.False(t, isUpstreamBillingProbeAccount(&Account{Platform: PlatformWorkbuddy, Type: AccountTypeAPIKey}))
}

func TestUpstreamBillingProbeIdentityExcludesQoder(t *testing.T) {
	require.False(t, IsUpstreamBillingProbeIdentity(PlatformQoder, AccountTypeAPIKey))
	require.False(t, isUpstreamBillingProbeAccount(&Account{Platform: PlatformQoder, Type: AccountTypeAPIKey}))
}
...
func TestUpstreamBillingProbeGrokAccountPersistsSnapshot(t *testing.T) {
	...
	require.Equal(t, "Bearer tk-grok-relay", upstream.lastReq.Header.Get("Authorization"))
...
func TestUpstreamBillingProbeNonOpenAIWithoutBaseURLIsUnsupportedWithoutRequest(t *testing.T) {
	account := &Account{
		...
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Credentials: map[string]any{"api_key": "tk-grok-official"},
	}
	repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{account.ID: account}}
...
func TestUpstreamBillingProbeOfficialAPIBaseURLIsUnsupportedWithoutRequest(t *testing.T) {
	...
	for i, tc := range cases {
		account := &Account{
			...
			Type:        AccountTypeAPIKey,
			Status:      StatusActive,
			Credentials: map[string]any{"api_key": "tk-official", "base_url": tc.baseURL},
		}
		repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{account.ID: account}}
		upstream := &httpUpstreamRecorder{}
		svc := newUpstreamBillingProbeTestService(repo, upstream, &upstreamBillingProbeSettingRepo{})
...
// WorkBuddy 凭据为 access/refresh token（无 credentials.api_key），探测取数必然
// missing_api_key 失败。平台不具备探测资格后的三条路径：
//   - 手动探测直接拒绝；
//   - 开关入口拒绝；
//   - 调度器对存量 enabled=true 的 workbuddy 账号静默跳过（不发请求、不写快照），
//     同批合格账号照常探测。
func TestUpstreamBillingProbeWorkbuddyAccountsAreSkipped(t *testing.T) {
	workbuddy := &Account{
		ID:       161,
	...
	openai := &Account{
		...
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Credentials: map[string]any{"api_key": "sk", "base_url": "https://relay.example/v1"},
		Extra:       map[string]any{UpstreamBillingProbeEnabledExtraKey: true},
	}
	repo := &upstreamBillingProbeAccountRepo{accounts: map[int64]*Account{
	...
	_, err := svc.ProbeAccount(context.Background(), workbuddy.ID)
	require.ErrorIs(t, err, ErrUpstreamBillingProbeAccountInvalid)

	err = svc.SetAccountEnabled(context.Background(), workbuddy.ID, true)
	require.ErrorIs(t, err, ErrUpstreamBillingProbeAccountInvalid)

	require.NoError(t, svc.RunDue(context.Background()))
	require.NotNil(t, upstream.lastReq, "eligible account in the same cycle must still be probed")
	require.Contains(t, upstream.lastReq.URL.String(), "relay.example")
	require.NotContains(t, upstream.lastReq.URL.String(), "copilot.tencent.com")
	require.NotContains(t, workbuddy.Extra, UpstreamBillingProbeExtraKey)
}
