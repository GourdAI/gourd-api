...
func (s *ChannelService) ListAvailable(ctx context.Context) ([]AvailableChannel, error) {
	...
	fullGroupByID := make(map[int64]*Group, len(groups))
	for i := range groups {
		g := groups[i]
		fullGroupByID[g.ID] = &groups[i]
	...
	for i := range channels {
		...
		channelGroups := make([]*Group, 0, len(ch.GroupIDs))
		for _, gid := range ch.GroupIDs {
			if g, ok := fullGroupByID[gid]; ok {
				channelGroups = append(channelGroups, g)
			}
		}
		supported = s.filterUnpricedSupportedModels(ctx, supported, channelGroups)
...
// filterUnpricedSupportedModels 剔除「可用渠道」展示中无法解析定价的模型。
// 判定遍历渠道关联的活跃分组（分组自定义价 → 渠道价 → 全局价），全未命中即未定价。
// 闸门未接线或开关关闭时原样返回。
func (s *ChannelService) filterUnpricedSupportedModels(ctx context.Context, models []SupportedModel, groups []*Group) []SupportedModel {
	gate := s.modelPricingGate
	if gate == nil || !gate.Enabled() || len(models) == 0 {
		return models
	}
	filtered := make([]SupportedModel, 0, len(models))
	for _, model := range models {
		if !gate.HasPricingForGroups(ctx, model.Name, groups) {
			continue
		}
