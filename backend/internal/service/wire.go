...
// ProvideAntigravityGatewayService 构造 Antigravity 网关服务并注入 TLS 指纹模板服务（防封加固）。
...
// ProvideGeminiMessagesCompatService 构造 Gemini 兼容服务并注入 TLS 指纹模板服务（防封加固）。
...
// ProvideWorkBuddyCreditsService 构造 WorkBuddy 积分查询/签到服务。
func ProvideWorkBuddyCreditsService(
	accountRepo AccountRepository,
	proxyRepo ProxyRepository,
	httpUpstream HTTPUpstream,
	cfg *config.Config,
) *WorkBuddyCreditsService {
	return NewWorkBuddyCreditsService(accountRepo, proxyRepo, httpUpstream, cfg)
}

// ProvideWorkBuddyCheckinService 构造并启动每日签到任务。
// 触发小时取自 gateway.workbuddy.checkin_hours（默认 [9,21]）；
// checkin_enabled=false 或小时列表全非法时不启动。
func ProvideWorkBuddyCheckinService(
	creditsService *WorkBuddyCreditsService,
	accountRepo AccountRepository,
	cfg *config.Config,
) *WorkBuddyCheckinService {
	svc := NewWorkBuddyCheckinService(creditsService, accountRepo, cfg)
	svc.Start()
	return svc
}

// ProvideQoderCreditsService 构造 Qoder 积分查询/活动 Credits 领取服务。
func ProvideQoderCreditsService(
	accountRepo AccountRepository,
	proxyRepo ProxyRepository,
	httpUpstream HTTPUpstream,
	cfg *config.Config,
) *QoderCreditsService {
	return NewQoderCreditsService(accountRepo, proxyRepo, httpUpstream, cfg)
}

// ProvideQoderCheckinService 构造并启动 Qoder 每日活动 Credits 领取任务。
// 触发小时取自 gateway.qoder.checkin_hours（默认 [10,21]，对齐活动 10:00 开放的
// 轮次口径）；checkin_enabled=false 或小时列表全非法时不启动。
func ProvideQoderCheckinService(
	creditsService *QoderCreditsService,
	accountRepo AccountRepository,
	cfg *config.Config,
) *QoderCheckinService {
	svc := NewQoderCheckinService(creditsService, accountRepo, cfg)
	svc.Start()
	return svc
}
...
// 对应 enabled=false 或小时列表全非法时该类 loop 不启动（另一类仍按自身门控启动）。
func ProvideWorkBuddyTasksService(
	creditsService *WorkBuddyCreditsService,
	accountRepo AccountRepository,
	cfg *config.Config,
) *WorkBuddyTasksService {
	svc := NewWorkBuddyTasksService(creditsService, accountRepo, cfg)
	svc.Start()
	return svc
}
...
func ProvideRateLimitService(
	...
	svc.SetRateLimit429CounterCache(rateLimit429CounterCache)
...
var ProviderSet = wire.NewSet(
	...
	ProvideWorkBuddyCreditsService,
	ProvideWorkBuddyCheckinService,
	ProvideWorkBuddyTasksService,
	ProvideQoderCreditsService,
	ProvideQoderCheckinService,
