...
func (s *AntigravityGatewayService) SetTLSFingerprintProfileService(svc *TLSFingerprintProfileService) {
	s.tlsFPProfileService = svc
}

// resolveAntigravityTLSProfile 解析账号的出站 TLS Profile（未启用/未注入时 nil）。
func (s *AntigravityGatewayService) resolveAntigravityTLSProfile(account *Account) *tlsfingerprint.Profile {
	return resolveAccountTLSProfile(s.tlsFPProfileService, account)
}
...
func (s *AntigravityGatewayService) TestConnection(ctx context.Context, account *Account, modelID string) (*TestConnectionResult, error) {
	...
	p := antigravityRetryLoopParams{
		...
		tlsProfile:     s.resolveAntigravityTLSProfile(account),
