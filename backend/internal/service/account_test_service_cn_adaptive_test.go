...
func TestAccountTestService_AdaptiveChatOnlyProvidersTestChatAndAnthropicEndpoints(t *testing.T) {
	...
	require.Equal(t, "Bearer tk-adaptive-test", upstream.requests[0].Header.Get("Authorization"))
	require.Equal(t, "tk-adaptive-test", upstream.requests[1].Header.Get("x-api-key"))
...
func TestAccountTestService_AdaptiveDeepSeekAlsoTestsResponsesEndpoint(t *testing.T) {
	...
	require.Equal(t, "Bearer tk-adaptive-test", upstream.requests[2].Header.Get("Authorization"))
...
func TestAccountTestService_AnthropicProtocolProbesNativeEndpointWithoutBetaQuery(t *testing.T) {
	...
	require.Equal(t, "tk-anthropic-test", req.Header.Get("x-api-key"))
