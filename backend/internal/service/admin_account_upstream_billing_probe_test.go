...
// WorkBuddy 凭据为 token（无 api_key），不具备探测资格：创建/更新入口
// 携带 probe_enabled=true 时必须拒绝，避免写入永远探测失败的开关。
func TestCreateAndUpdateAccountRejectProbeEnabledForWorkbuddy(t *testing.T) {
	enabled := true
	_, err := (&adminServiceImpl{accountRepo: &upstreamBillingProbeAccountRepo{}}).CreateAccount(context.Background(), &CreateAccountInput{
		Name:                 "wb",
	...
	repo := &upstreamBillingProbeAdminRepo{upstreamBillingProbeAccountRepo: &upstreamBillingProbeAccountRepo{
		accounts: map[int64]*Account{wb.ID: wb},
	}}
	_, err = (&adminServiceImpl{accountRepo: repo}).UpdateAccount(context.Background(), wb.ID, &UpdateAccountInput{
		ProbeEnabled: &enabled,
	})
	require.ErrorIs(t, err, ErrUpstreamBillingProbeAccountInvalid)
}
