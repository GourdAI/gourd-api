...
func TestAllowOpenAICompatibleMessagesDispatch_CNProvidersExempt(t *testing.T) {
	...
	for _, platform := range []string{service.PlatformKimi, service.PlatformZhipu, service.PlatformDeepseek, service.PlatformMiniMax, service.PlatformOpenCodeGo, service.PlatformGrok, service.PlatformWorkbuddy, service.PlatformQoder} {
...
func TestAllowOpenAICompatibleMessagesDispatch_CompositeResolvedTargets(t *testing.T) {
	...
	// 解析到 workbuddy / qoder 目标：与对应独立分组同语义豁免。
	cWB, _ := gin.CreateTestContext(httptest.NewRecorder())
	cWB.Request = httptest.NewRequest("POST", "/v1/messages", nil)
	cWB.Request = cWB.Request.WithContext(service.WithResolvedTargetPlatform(cWB.Request.Context(), service.PlatformWorkbuddy))
	require.True(t, allowOpenAICompatibleMessagesDispatch(cWB,
		&service.APIKey{Group: &service.Group{Platform: service.PlatformComposite, AllowMessagesDispatch: false}}))

	cQoder, _ := gin.CreateTestContext(httptest.NewRecorder())
	cQoder.Request = httptest.NewRequest("POST", "/v1/messages", nil)
	cQoder.Request = cQoder.Request.WithContext(service.WithResolvedTargetPlatform(cQoder.Request.Context(), service.PlatformQoder))
	require.True(t, allowOpenAICompatibleMessagesDispatch(cQoder,
		&service.APIKey{Group: &service.Group{Platform: service.PlatformComposite, AllowMessagesDispatch: false}}))
