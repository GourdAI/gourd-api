...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	// 两级过滤同一个出口：分组白名单（若开启）与未定价模型（开关关闭时不过滤）。
	// models[].name 形如 models/xxx，定价查找与白名单都对齐到裸模型名。
	filterGeminiModels := func(models []gemini.Model) []gemini.Model {
		group := apiKey.Group
		// 定价判定用候选并集（任一候选分组算得出价即可见），与准入闸门同源；
		// 白名单仍按生效分组判定（它是「本分组准不准用」的语义，不做并集）。
		groups := service.PricingCandidateGroups(apiKey)
		gate := h.pricingGate()
		filtered := make([]gemini.Model, 0, len(models))
		for _, model := range models {
			// 白名单保持原有判定口径（传入原始 name，含 models/ 前缀）。
			if group != nil && group.ModelAllowlistEnabled() && !group.ModelAllowlist.Allows(model.Name) {
				continue
			}
			// 定价查找对齐裸模型名（去 models/ 前缀）。
			if gate.Enabled() && !gate.HasPricingForGroups(c.Request.Context(), trimModelPrefix(model.Name), groups) {
				continue
			}
