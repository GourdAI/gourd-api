...
// apiKeyGroupIDsForDTO 输出候选分组集合（主分组优先，始终非 nil 以符合前后端契约），
// 兼容三种状态：多分组（GroupIDs 已填充）、单分组（退化为主分组）、无绑定（空数组）。
func apiKeyGroupIDsForDTO(k *service.APIKey) []int64 {
	if k == nil {
		return []int64{}
	}
	if len(k.GroupIDs) > 0 {
...
func APIKeyFromService(k *service.APIKey) *APIKey {
	...
	out := &APIKey{
		...
		Name:               k.Name,
		GroupID:            k.GroupID,
		GroupIDs:           apiKeyGroupIDsForDTO(k),
		Status:             k.Status,
