...
// TestAccountFromServiceShallow_TLSFingerprintAllPlatforms 验证 TLS 指纹开关与模板 ID 的
// 提取不再受 Anthropic 限制：防封加固后该配置对所有平台生效（默认关闭 opt-in）。
func TestAccountFromServiceShallow_TLSFingerprintAllPlatforms(t *testing.T) {
	platforms := []string{
	...
	for _, platform := range platforms {
		t.Run(platform, func(t *testing.T) {
			// 已开启并绑定模板：应输出开关与模板 ID
			enabled := &service.Account{
				ID: 1, Platform: platform, Type: service.AccountTypeOAuth,
				Extra: map[string]any{
					"enable_tls_fingerprint":     true,
					"tls_fingerprint_profile_id": 7,
				},
			}
			got := AccountFromServiceShallow(enabled)
			...
			require.Equal(t, int64(7), *got.TLSFingerprintProfileID)

			// 未配置：默认关闭，不输出任何 TLS 字段
			disabled := &service.Account{ID: 2, Platform: platform, Type: service.AccountTypeOAuth}
			gotDisabled := AccountFromServiceShallow(disabled)
			require.Nil(t, gotDisabled.EnableTLSFingerprint)
			require.Nil(t, gotDisabled.TLSFingerprintProfileID)

			// 随机模板（-1）保持不输出，行为与改动前一致
			randomProfile := &service.Account{
				ID: 3, Platform: platform, Type: service.AccountTypeOAuth,
				Extra: map[string]any{
					"enable_tls_fingerprint":     true,
					"tls_fingerprint_profile_id": -1,
				},
			}
			gotRandom := AccountFromServiceShallow(randomProfile)
			require.NotNil(t, gotRandom.EnableTLSFingerprint)
			require.Nil(t, gotRandom.TLSFingerprintProfileID)
		})
	}
}

// TestAccountFromServiceShallow_TLSFingerprintIndependentOfAnthropicOnlyFields 验证
// TLS 字段不再依赖 Anthropic 专属字段（如 window_cost_limit）作为前置条件。
func TestAccountFromServiceShallow_TLSFingerprintIndependentOfAnthropicOnlyFields(t *testing.T) {
	src := &service.Account{
		ID: 4, Platform: service.PlatformOpenAI, Type: service.AccountTypeAPIKey,
		Extra: map[string]any{"enable_tls_fingerprint": true},
	}
	got := AccountFromServiceShallow(src)
	require.NotNil(t, got.EnableTLSFingerprint)
	require.True(t, *got.EnableTLSFingerprint)
	// API Key 账号不参与 Anthropic 专属配置，但 TLS 字段必须仍然可见
	require.Nil(t, got.WindowCostLimit)
	require.Nil(t, got.MaxSessions)
}
