...
//
// 多分组绑定：返回绑定分组的模型并集（再经各分组白名单/定价过滤）。
// 单分组 key 行为与历史完全一致（并集退化为单分组结果）。
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	// 多分组绑定（候选 >1）时，合并**全部**绑定分组的模型列表（不按平台裁剪）。
	// 网关对各上游做协议转换，任一绑定分组能服务的模型都能用本 key 当前协议调用，
	// 因此列表必须与实际可调用集合一致（详见 multiGroupModelUnion 注释）。
	if unionModels := h.multiGroupModelUnion(c, apiKey); len(unionModels) > 0 {
		// unionModels 已在 multiGroupModelUnion 里逐个分组做完两道过滤：
		//   - 定价：按本分组上下文判（任一分组算得出价即保留）；
		//   - 白名单：按本分组自己的 ModelAllowlist 判。
		// 此处不得再拿「主分组」的定价或白名单重过滤：多分组语义是各分组口径取并，
		// 用主分组（unionGroup=eligible[0]）二次裁剪会把「只在另一个分组配了价/被另一个
		// 分组白名单放行」的模型误删，重新制造出本次要修的「能调但不列」。
		// 输出 shape 仍按请求侧 platform（客户端用哪个协议访问就拿哪种列表）。
		writeModelsList(c, platform, unionModels)
		return
	}
	...
	if platform == service.PlatformComposite {
		...
		availableModels = h.filterPricedModelsForKey(c.Request.Context(), availableModels, apiKey)
		if apiKey != nil && apiKey.Group != nil && apiKey.Group.ModelAllowlistEnabled() {
			source := availableModels
			if len(source) == 0 {
				source = defaultModelIDsForPlatform(service.PlatformComposite)
			}
			source = h.filterPricedModelsForKey(c.Request.Context(), source, apiKey)
		...
		writeModelsList(c, service.PlatformComposite, h.filterPricedModelsForKey(c.Request.Context(), defaultModelIDsForPlatform(service.PlatformComposite), apiKey))
		return
	}
	...
	// Fallback to default models（兜底列表同样要过滤，否则未定价模型会从这条路漏出）。
	if platform == service.PlatformOpenAI {
		writeModelsListResponse(c, h.filterPricedOpenAIModels(c, openai.DefaultModels))
		return
	}

	if platform == service.PlatformGemini {
		writeModelsListResponse(c, h.filterPricedGeminiModels(c, geminicli.DefaultModels))
		return
	}
	if platform == service.PlatformGrok {
		writeGrokModelsList(c, h.filterPricedModelsForKey(c.Request.Context(), xai.DefaultModelIDs(), apiKey))
		return
	}

	writeModelsListResponse(c, h.filterPricedClaudeModels(c, claude.DefaultModels))
}

// multiGroupModelUnion 在 key 绑定多个分组时，返回各分组可用模型的并集。
//
// 并集**不按平台裁剪**：绑定几个分组就合并几个分组的模型。
//
// 历史实现只合并「与主分组 platform 相同」的分组，给出的理由是「避免客户端拿到无法用
// 当前协议调用的模型」。该理由在本仓架构下不成立：网关对外统一暴露 OpenAI / Anthropic /
// Gemini 兼容入口并对各上游做协议转换，任一绑定分组能服务的模型都能用同一把 key、同一
// 协议调用成功。2026-09-24 端到端实测：主分组 qoder + 候选 workbuddy 的 key 请求 workbuddy
// 专属 deepseek-v4.1-flash → HTTP 200，且 usage_logs.group_id 正确落到 workbuddy 分组；
// 而当时 /v1/models 不列出该模型，形成「能调但不列」——靠 /v1/models 自动发现模型的
// 客户端（Cursor / Cline 等）看不见实际可用的跨平台模型。
//
// 返回并集模型列表；单分组 key、候选全未启用、无可用模型时返回 nil 以回退既有路径。
// 定价与白名单两道过滤均在本函数内按**各分组自己的口径**完成，调用方不得再拿
// 任何一个分组去二次裁剪（那等于把并集降级回单分组语义）。
func (h *GatewayHandler) multiGroupModelUnion(c *gin.Context, apiKey *service.APIKey) []string {
	if h == nil || h.gatewayService == nil || apiKey == nil || !service.HasMultipleCandidateGroups(apiKey) {
		return nil
	}
	candidates := apiKey.Groups
	if len(candidates) == 0 {
		return nil
	}

	// 仅剔除 nil 与未启用分组：inactive 分组不能服务请求，其模型不得列入。
	...
	for _, group := range eligible {
		models := h.groupAvailableModelsForUnion(c, group)
		// 定价过滤必须按「本分组」上下文在合并前做：多分组语义是「任一分组
		// 能服务且算得出价即可列出」。若留到外层用某一个分组统一过滤，会把
		// 「只在 B 组配了价」的模型误删。开关关闭时 FilterPriced 原样返回。
		models = gate.FilterPriced(c.Request.Context(), models, group)
...
// groupAvailableModelsForUnion 取单个绑定分组可服务的模型列表。
//
// composite 分组必须单独走多平台展开：它的账号平台是若干真实平台（而非 "composite"），
// 若按 group.Platform 去 GetAvailableModels 过滤账号，会因平台不匹配而取到空集，
// 使该分组在并集中“隐形”（与不合并前相比更糟：它单绑时有专用分支能列出全平台模型）。
func (h *GatewayHandler) groupAvailableModelsForUnion(c *gin.Context, group *service.Group) []string {
	groupID := group.ID
	if group.Platform == service.PlatformComposite {
		return h.compositeAvailableModels(c.Request.Context(), &groupID)
	}
	return h.gatewayService.GetAvailableModels(c.Request.Context(), &groupID, group.Platform)
}
...
func (h *GatewayHandler) CodexModels(c *gin.Context) {
	...
	// 未定价模型不出名单：与 HTTP 入口拒绝对齐，避免 Codex 端选到必 404 的模型。
	modelIDs = h.filterPricedModelsForKey(c.Request.Context(), modelIDs, apiKey)
...
func defaultModelIDsForPlatform(platform string) []string {
	...
	case service.PlatformTrae:
		return service.DefaultTraeModelIDs()
...
func (h *GatewayHandler) AntigravityModels(c *gin.Context) {
	...
	// 未定价模型不上架（开关关闭时原样返回）；多分组 Key 按候选并集判定。
	if gate := h.pricingGate(); gate.Enabled() {
		groups := service.PricingCandidateGroups(apiKey)
		filtered := make([]antigravity.ClaudeModel, 0, len(models))
		for _, model := range models {
...
// SchedulingService 暴露网关调度服务实例，供路由层注册「生效分组决议」中间件使用
// （决议需要探测分组内账号能否服务指定模型）。
func (h *GatewayHandler) SchedulingService() *service.GatewayService {
	if h == nil {
		return nil
	}
