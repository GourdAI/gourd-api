-- Add WorkBuddy as a first-class platform (OpenAI-compatible provider).
--
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('workbuddy' IN monitor_constraint_def) = 0 THEN
        ALTER TABLE channel_monitors
            DROP CONSTRAINT IF EXISTS channel_monitors_provider_check;
        ALTER TABLE channel_monitors
            ADD CONSTRAINT channel_monitors_provider_check
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok',
                                'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'workbuddy'));
    END IF;
    ...
    IF template_constraint_def IS NULL OR position('workbuddy' IN template_constraint_def) = 0 THEN
