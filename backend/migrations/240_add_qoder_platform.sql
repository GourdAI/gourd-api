-- Add Qoder as a first-class platform (COSY-protocol provider).
--
...
--
-- Runs after 239_add_workbuddy_platform.sql. DROP ... IF EXISTS + 幂等守卫保证可重入；
-- 新约束是 239 的超集，必须同时保留 MiniMax、OpenCode GO 与 WorkBuddy。
...
ALTER TABLE user_platform_quotas
    ...
    CHECK (platform IN ('anthropic', 'openai', 'gemini', 'antigravity', 'grok',
                        'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'workbuddy', 'qoder'));
...
BEGIN
    ...
    IF monitor_constraint_def IS NULL OR position('qoder' IN monitor_constraint_def) = 0 THEN
        ALTER TABLE channel_monitors
            DROP CONSTRAINT IF EXISTS channel_monitors_provider_check;
        ALTER TABLE channel_monitors
            ADD CONSTRAINT channel_monitors_provider_check
            CHECK (provider IN ('openai', 'anthropic', 'gemini', 'grok',
                                'antigravity', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go', 'workbuddy', 'qoder'));
    END IF;
    ...
    IF template_constraint_def IS NULL OR position('qoder' IN template_constraint_def) = 0 THEN
