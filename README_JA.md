...
# 独自に修正したソースをデプロイする場合は、チェックアウト先でビルドします：
#   cd sub2api && bash deploy/docker-deploy-local.sh

# ログを表示
# （スクリプトがすでにサービスを起動済みです）
docker compose logs -f sub2api
```

**スクリプトの動作内容:**
- スクリプトに同梱されたテンプレートから `docker-compose.yml` と `.env.example` を生成（デプロイ時に GitHub から何もダウンロードしません）
...
- サービスを自動起動し、アプリケーションのヘルスチェック通過まで待機
- 生成された認証情報を参照用に表示
