...
### 坑 11：一键部署脚本的内嵌模板必须同步（改了 compose/.env.example 之后）

**背景**：`deploy/docker-deploy.sh` 把 `deploy/docker-compose.local.yml` 和 `deploy/.env.example` 的完整内容内嵌在脚本里（作为 `SUB2API_COMPOSE_TEMPLATE_EOF` / `SUB2API_ENV_TEMPLATE_EOF` 两个 heredoc 块），部署执行阶段不再从 GitHub 拉取任何文件。

**问题**：改了 `docker-compose.local.yml` 或 `.env.example` 而忘记同步内嵌副本时，一键部署生成的配置就是旧的。

**解决**：
```bash
sh deploy/tools/sync-bundled-templates.sh   # 重新生成内嵌副本
sh deploy/tests/docker-deploy-test.sh       # 校验（CI 同样会跑，漂移即失败）
```

注意：
- 内嵌块内容必须与 canonical 文件**字节一致**（包括空行与行尾空格），不要手工编辑脚本里的模板区域。
- 脚本在仓库 `deploy/` 目录内运行时会拒绝执行（防止覆盖受版本控制的文件），测试部署时请在临时空目录中运行。
- **重要区分**：`docker-deploy.sh` 部署的是 Docker Hub 上的**官方镜像** `weishaw/sub2api:latest`，永远不包含本仓库的改动。要部署自己改过的代码，用 `deploy/docker-deploy-local.sh`：它读取 canonical 的 `docker-compose.local.yml` / `.env.example`（无内嵌副本、无需同步），把 `image:` 改写为本地构建的 `sub2api:local` 并注入 `build.context=<仓库根>`，再 `docker compose build` + `up -d`。因为 `sub2api:local` 在任何 registry 都不存在，不可能被误拉成上游镜像。
...
- [ ] 部署脚本改动已跑 `sh deploy/tests/docker-deploy-test.sh`（含内嵌模板同步检查）
- [ ] 本地源码部署脚本改动已跑 `sh deploy/tests/docker-deploy-local-test.sh`
