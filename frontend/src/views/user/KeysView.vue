  ...
  <AppLayout>
          ...
          <template #cell-group="{ row }">
            <div class="group/dropdown relative">
              <button
                ...
                class="-mx-2 -my-1 flex cursor-pointer items-center gap-1 rounded-lg px-2 py-1 transition-all duration-200 hover:bg-gray-100 dark:hover:bg-dark-700"
                :title="t('keys.clickToChangeGroup')"
                data-test="key-group-cell"
              >
                <template v-if="visibleKeyGroups(row).length">
                  <GroupBadge
                    v-for="entry in visibleKeyGroups(row)"
                    :key="entry.id"
                    :name="entry.name"
                    :platform="entry.group?.platform"
                    :subscription-type="entry.group?.subscription_type"
                    :rate-multiplier="entry.group?.rate_multiplier"
                    :user-rate-multiplier="entry.group ? userGroupRates[entry.group.id] : null"
                  ...
                  />
                  <span
                    v-if="hiddenKeyGroupCount(row) > 0"
                    class="shrink-0 rounded-md bg-gray-100 px-1.5 py-0.5 text-xs font-medium text-gray-600 dark:bg-dark-700 dark:text-gray-300"
                    :title="t('keys.moreGroups', { count: hiddenKeyGroupCount(row) }) + ': ' + hiddenKeyGroupNames(row)"
                    data-test="key-group-more"
                  >
                    +{{ hiddenKeyGroupCount(row) }}
                  </span>
                </template>
      ...
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        ...
        <div>
          ...
          <MultiGroupSelector
            ...
            :groups="formGroupOptions"
            :user-group-rates="userGroupRates"
            :searchable="true"
            data-tour="key-form-group"
            test-id="key-form-group"
          />
          <p class="input-hint mt-1">{{ t('keys.primaryGroupHint') }}</p>
        </div>
    ...
    <Teleport to="body">
      ...
      >
        ...
        <!-- Group list (multi-select draft, committed with the save button) -->
        <div class="max-h-80 overflow-y-auto p-1.5">
          <label
            v-for="option in filteredGroupOptions"
            :key="option.value"
            class="flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2.5 text-sm transition-colors"
            :class="[
              'border-b border-gray-100 last:border-0 dark:border-dark-700',
              groupSelectorDraft.includes(option.value)
                ? 'bg-primary-50 dark:bg-primary-900/20'
                : 'hover:bg-gray-100 dark:hover:bg-dark-700'
            ]"
            :title="option.description || undefined"
            :data-test="`inline-group-option-${option.value}`"
          >
            <input
              type="checkbox"
              class="h-4 w-4 shrink-0 rounded border-gray-300 text-primary-600 focus:ring-primary-500/40"
              :checked="groupSelectorDraft.includes(option.value)"
              @click.stop
              @change="toggleDraftGroup(option.value, ($event.target as HTMLInputElement).checked)"
            />
            <GroupOptionItem
              :name="option.label"
              ...
              :selected="groupSelectorDraft.includes(option.value)"
              class="min-w-0 flex-1"
            />
            <span
              v-if="groupSelectorDraft[0] === option.value"
        ...
        <div class="flex items-center justify-between gap-2 border-t border-gray-100 p-2 dark:border-dark-700">
          <button
            ...
            :disabled="groupSelectorDraft.length === 0"
            data-test="inline-group-clear"
            @click.stop="groupSelectorDraft = []"
          >
            {{ t('keys.noGroup') }}
          </button>
          <div class="flex items-center gap-2">
            <span class="text-xs text-gray-500 dark:text-gray-400">
              {{ t('common.selectedCount', { count: groupSelectorDraft.length }) }}
            </span>
            <button
              type="button"
              class="btn btn-primary btn-sm"
              :disabled="savingGroups"
              data-test="inline-group-save"
              @click.stop="saveGroups(selectedKeyForGroup!)"
            >
              {{ savingGroups ? t('keys.saving') : t('common.save') }}
            </button>
          </div>
        </div>
      </div>
...
import MultiGroupSelector from '@/components/common/MultiGroupSelector.vue'
...
/** A single entry of an API key's bound group set. */
interface ApiKeyGroupEntry {
  id: number
  name: string
  group?: Group
}
...
/**
 * Resolve the displayed group set of a key.
 *
 * `group_ids` is the contract source of truth while `group_id`/`group` carry
 * the primary group for backwards compatibility. Names fall back to the
 * locally loaded group list so multi-group rows can render every badge.
 */
const resolveKeyGroups = (key: ApiKey): ApiKeyGroupEntry[] => {
  const ids = key.group_ids?.length
    ...
    : key.group_id != null
      ...
      : []
  return ids.map((id) => {
    const known = groups.value.find((group) => group.id === id)
...
const keyGroupsByKeyId = computed(() => {
  const map = new Map<number, ApiKeyGroupEntry[]>()
  for (const key of apiKeys.value) map.set(key.id, resolveKeyGroups(key))
  return map
})

/** Groups rendered inline in the table cell before collapsing into "+N". */
const MAX_VISIBLE_KEY_GROUPS = 1

const visibleKeyGroups = (key: ApiKey): ApiKeyGroupEntry[] =>
  (keyGroupsByKeyId.value.get(key.id) ?? []).slice(0, MAX_VISIBLE_KEY_GROUPS)

const hiddenKeyGroupCount = (key: ApiKey): number =>
  Math.max(0, (keyGroupsByKeyId.value.get(key.id) ?? []).length - MAX_VISIBLE_KEY_GROUPS)

const hiddenKeyGroupNames = (key: ApiKey): string =>
  (keyGroupsByKeyId.value.get(key.id) ?? [])
    .slice(MAX_VISIBLE_KEY_GROUPS)
...
/** Draft group selection of the inline dropdown; committed by `saveGroups`. */
...
const formData = ref({
  name: '',
  /** Bound groups; the first entry is the primary group sent as `group_ids[0]`. */
  group_ids: [] as number[],
...
watch([showCreateModal, createProviderOptions], ([isOpen, providers], [wasOpen]) => {
  ...
  // Drop selections that are no longer offered by the active provider filter.
  const availableIds = new Set(formGroupOptions.value.map((group) => group.id))
  const kept = formData.value.group_ids.filter((id) => availableIds.has(id))
  if (kept.length !== formData.value.group_ids.length) {
    formData.value.group_ids = kept
  }
})
...
const editKey = (key: ApiKey) => {
  ...
  formData.value = {
    name: key.name,
    group_ids: resolveKeyGroups(key).map((entry) => entry.id),
...
const openGroupSelector = (key: ApiKey) => {
  if (groupSelectorKeyId.value === key.id) {
    ...
    groupSelectorDraft.value = resolveKeyGroups(key).map((entry) => entry.id)
  }
}
...
/**
 * Persist the inline group draft. Draft order defines the primary group: the
 * first selected id heads `group_ids`, and an empty draft unbinds every group
 * (`group_ids: []`).
 */
const saveGroups = async (key: ApiKey) => {
  const nextIds = [...groupSelectorDraft.value]
  const currentIds = resolveKeyGroups(key).map((entry) => entry.id)
  const unchanged =
  ...
  try {
    await keysAPI.update(key.id, { group_ids: nextIds })
    appStore.showSuccess(t('keys.groupsChangedSuccess'))
    groupSelectorKeyId.value = null
    dropdownPosition.value = null
    loadApiKeys()
  } catch (error) {
    appStore.showError(t('keys.failedToChangeGroups'))
  } finally {
...
const handleSubmit = async () => {
  // At least one group is required for both create and edit.
  if (formData.value.group_ids.length === 0) {
    appStore.showError(t('keys.groupsRequired'))
    return
  }
