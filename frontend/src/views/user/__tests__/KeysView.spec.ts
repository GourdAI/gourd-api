...
const messages: Record<string, string> = {
  ...
  'admin.groups.rateAndAccounts': 'rate {rate} accounts {count}',
  'keys.apiKey': 'API Key',
  ...
  'keys.groupsLabel': 'Groups',
  'keys.selectGroups': 'Select groups',
  'keys.searchGroups': 'Search groups...',
  'keys.noGroupsFound': 'No groups found',
  'keys.primaryGroupBadge': 'Primary',
  'keys.primaryGroupHint': 'The first group is the primary group',
  'keys.usage': 'Usage',
  'keys.saving': 'Saving...',
  'keys.pickGroup': 'Select a group',
}
vi.mock('@/api', () => ({
  keysAPI: {
    list: listKeys,
...
const DataTableStub = {
  ...
  methods: {
    groupCellTest(row: { id: number }) {
      return `key-group-${row.id}`
    },
  },
  template: `
    <div>
      ...
      <div v-for="row in data" :key="row.id">
        ...
        <div :data-test="groupCellTest(row)">
...
describe('user KeysView column settings', () => {
  ...
  describe('create provider selection', () => {
    ...
    const selectedIds = (wrapper: VueWrapper) => groupSelect(wrapper).props('modelValue')
    ...
    it('classifies all configured platforms and retains the complete table filter', async () => {
      ...
      // The table filter keeps the complete group list.
      expect(wrapper.findAllComponents({ name: 'Select' })[0].props('options')).toHaveLength(13)
    })

    it('clears the previous selection on provider change and submits every chosen group', async () => {
      const wrapper = await openCreate()
      ...
      expect(showError).toHaveBeenCalledWith('keys.groupsRequired')

      // Select two groups: the order defines the primary group.
      await groupSelect(wrapper).vm.$emit('update:modelValue', [5, 6])
      vi.mocked(keysAPI.create).mockResolvedValue({ ...createApiKey(), group_id: 5, group_ids: [5, 6] })
      await wrapper.get('#key-form').trigger('submit')
      await flushPromises()
      expect(keysAPI.create).toHaveBeenCalledOnce()
      expect(vi.mocked(keysAPI.create).mock.calls[0].slice(0, 2)).toEqual(['My key', [5, 6]])
    })
  ...
  describe('multi-group display and rebinding', () => {
    const groupCatalog = [
      { id: 1, name: 'Alpha', platform: 'anthropic' },
      { id: 2, name: 'Beta', platform: 'openai' },
      { id: 3, name: 'Gamma', platform: 'gemini' },
    ]

    beforeEach(() => {
      getAvailableGroups.mockResolvedValue(groupCatalog)
    })

    it('shows the primary badge plus a "+N" counter for multi-group keys', async () => {
      listKeys.mockResolvedValue({
        items: [{
          ...createApiKey(),
          group_id: 1,
          group_ids: [1, 2, 3],
          group: { id: 1, name: 'Alpha', platform: 'anthropic' },
        }],
        total: 1,
        page: 1,
        page_size: 20,
      ...
      const wrapper = await mountView()
      const cell = wrapper.get('[data-test="key-group-1"]')

      expect(cell.findAll('[data-test="group-badge"]').map((badge) => badge.text())).toEqual(['Alpha'])
      expect(cell.get('[data-test="key-group-more"]').text()).toBe('+2')
      expect(cell.get('[data-test="key-group-more"]').attributes('title')).toBe('keys.moreGroups: Beta, Gamma')
      wrapper.unmount()
    })

    it('shows a single badge and no counter when only one group is bound', async () => {
      listKeys.mockResolvedValue({
        items: [{
          ...createApiKey(),
          group_id: 2,
          group_ids: [2],
          group: { id: 2, name: 'Beta', platform: 'openai' },
        }],
        total: 1,
        page: 1,
        page_size: 20,
      ...
      expect(cell.findAll('[data-test="group-badge"]').map((badge) => badge.text())).toEqual(['Beta'])
      expect(cell.find('[data-test="key-group-more"]').exists()).toBe(false)
      wrapper.unmount()
    })

    it('falls back to the legacy group_id when group_ids is absent', async () => {
      const legacy = { ...createApiKey(), group_id: 3, group: { id: 3, name: 'Gamma', platform: 'gemini' } }
      delete (legacy as { group_ids?: number[] }).group_ids
      listKeys.mockResolvedValue({ items: [legacy], total: 1, page: 1, page_size: 20, pages: 1 })

      const wrapper = await mountView()
      const cell = wrapper.get('[data-test="key-group-1"]')

      expect(cell.findAll('[data-test="group-badge"]').map((badge) => badge.text())).toEqual(['Gamma'])
      wrapper.unmount()
    })

    it('persists the inline multi-selection as group_ids with the newest pick last', async () => {
      listKeys.mockResolvedValue({
        items: [{
          ...createApiKey(),
          group_id: 1,
      ...
      await wrapper.get('[data-test="key-group-cell"]').trigger('click')
      await nextTick()
      await wrapper.get('[data-test="inline-group-option-2"] input').setValue(true)
      await nextTick()
      await wrapper.get('[data-test="inline-group-save"]').trigger('click')
      await flushPromises()

      expect(updateKey).toHaveBeenCalledWith(1, { group_ids: [1, 2] })
      expect(showSuccess).toHaveBeenCalledWith('keys.groupsChangedSuccess')
      wrapper.unmount()
    })

    it('sends an empty group_ids list when every group is cleared inline', async () => {
      listKeys.mockResolvedValue({
        items: [{
          ...createApiKey(),
          group_id: 1,
      ...
      await wrapper.get('[data-test="key-group-cell"]').trigger('click')
      await nextTick()
      await wrapper.get('[data-test="inline-group-clear"]').trigger('click')
      await nextTick()
      await wrapper.get('[data-test="inline-group-save"]').trigger('click')
      await flushPromises()

      expect(updateKey).toHaveBeenCalledWith(1, { group_ids: [] })
      wrapper.unmount()
    })

    it('skips the request when the inline selection is unchanged', async () => {
      listKeys.mockResolvedValue({
        items: [{
          ...createApiKey(),
          group_id: 1,
