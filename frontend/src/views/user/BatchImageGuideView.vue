...
const geminiApiKeys = computed(() =>
  apiKeys.value.filter((key) => {
    // Multi-group keys are filtered by their primary group (`group`), which the
    // backend keeps aligned with `group_ids[0]`.
    const primary = key.group
    return (
      key.status === 'active' &&
      primary?.platform === 'gemini' &&
      primary?.allow_batch_image_generation === true
    )
  }),
)
