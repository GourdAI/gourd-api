...
const handleBulkProbeUpstreamBilling = async () => {
  // 仅跳过「已加载到当前列表、且明确不具备探测资格」的账号（如 WorkBuddy，凭据为
  // token 无 api_key）：后端会对其逐条返回资格错误，混入选择会产生误导性的
  // 「部分完成/失败」提示。跨页选择但未加载到当前列表的 ID 无从判断，原样透传，
  // 交由后端逐条裁决（不能按 accounts.value 过滤，否则会静默丢失跨页选择）。
  const byId = new Map(accounts.value.map(account => [account.id, account]))
  const accountIDs: number[] = []
  let skippedCount = 0
  for (const id of selIds.value) {
    const account = byId.get(id)
    if (account && !isUpstreamBillingProbeCapable(account.platform, account.type)) {
      skippedCount += 1
      continue
    }
    accountIDs.push(id)
  ...
  if (skippedCount > 0) {
    appStore.showWarning(t('admin.accounts.upstreamBilling.batchSkippedIneligible', { count: skippedCount }))
  }
  if (accountIDs.length === 0) {
