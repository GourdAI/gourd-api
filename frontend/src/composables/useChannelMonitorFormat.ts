...
/**
 * 专有名词 provider 的硬编码显示名。这些平台尚无 `monitorCommon.providers.*`
 * 词条（不新增 i18n 键），直接展示品牌名避免 t() 回退成原始 key。
 */
const HARDCODED_PROVIDER_LABELS: Partial<Record<Provider, string>> = {
...
export function useChannelMonitorFormat() {
  ...
  function providerLabel(p: Provider | string): string {
    const hardcoded = HARDCODED_PROVIDER_LABELS[p as Provider]
