...
describe('workbuddy credentials', () => {
  it('builds snake_case credentials and skips empty optional fields', () => {
    const credentials = buildWorkbuddyCredentials({
      accessToken: ' wb-token ',
      refreshToken: '',
      realm: 'global',
      uid: ' wb-uid ',
      enterpriseId: '',
      deviceToken: 'wb-device',
    ...
    expect(credentials).toEqual({
      ...
      realm: 'global',
      uid: 'wb-uid',
      device_token: 'wb-device',
      base_url: 'https://www.workbuddy.ai'
    })
  })

  it('requires at least one of access_token / refresh_token', () => {
    expect(validateWorkbuddyCredentials('', '')).toBe(false)
    expect(validateWorkbuddyCredentials('wb-token', '')).toBe(true)
    expect(validateWorkbuddyCredentials('', 'wb-refresh')).toBe(true)
  })

  it('resolves realm defaults and CN/global base urls', () => {
    expect(resolveWorkBuddyRealm(undefined)).toBe('cn')
    expect(resolveWorkBuddyRealm('cn')).toBe('cn')
    expect(resolveWorkBuddyRealm('global')).toBe('global')
    expect(resolveWorkBuddyRealm('bogus')).toBe('cn')
    expect(defaultWorkBuddyBaseUrl()).toBe('https://copilot.tencent.com')
    expect(defaultWorkBuddyBaseUrl('cn')).toBe('https://copilot.tencent.com')
    // 国际默认端点必须带 www：裸域会被上游 301（POST→GET 改写）→ 404 page not found。
    expect(defaultWorkBuddyBaseUrl('global')).toBe('https://www.workbuddy.ai')
  })
})
...
// 探测资格与后端 IsUpstreamBillingProbeIdentity 对齐：非 apikey 类型不合格，
// 且 WorkBuddy（凭据为 token，无 credentials.api_key）已被后端移出名单。
describe('isUpstreamBillingProbeCapable', () => {
  it('accepts API-key accounts of probe-capable platforms', () => {
    for (const platform of ['openai', 'anthropic', 'gemini', 'antigravity', 'grok', 'kimi', 'zhipu', 'deepseek', 'minimax', 'opencode_go']) {
      expect(isUpstreamBillingProbeCapable(platform, 'apikey')).toBe(true)
    }
  })

  it('rejects WorkBuddy even for apikey accounts', () => {
    expect(isUpstreamBillingProbeCapable('workbuddy', 'apikey')).toBe(false)
    expect(isUpstreamBillingProbeCapable('workbuddy', 'oauth')).toBe(false)
  })

  it('rejects non-apikey account types', () => {
    expect(isUpstreamBillingProbeCapable('openai', 'oauth')).toBe(false)
    expect(isUpstreamBillingProbeCapable('anthropic', 'bedrock')).toBe(false)
    expect(isUpstreamBillingProbeCapable('grok', 'oauth')).toBe(false)
  })

  it('rejects unknown or empty platforms', () => {
    expect(isUpstreamBillingProbeCapable('', 'apikey')).toBe(false)
    expect(isUpstreamBillingProbeCapable('future-platform', 'apikey')).toBe(false)
  })
})
