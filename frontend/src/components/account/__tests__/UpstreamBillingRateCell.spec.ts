...
describe('UpstreamBillingRateCell', () => {
  ...
  it('emits manual probe commands only for eligible accounts', async () => {
    ...
    // WorkBuddy 凭据为 token（无 credentials.api_key），后端已将其移出探测资格名单：
    // 必须一并隐藏整列与手动探测按钮，否则点击会被后端 400 拒绝。
    await wrapper.setProps({ account: makeAccount({ platform: 'workbuddy', type: 'apikey' }) })
    expect(wrapper.findAll('button')).toHaveLength(0)
    expect(wrapper.text()).toBe('-')

    // Qoder 凭据同为 token（无 credentials.api_key），与 WorkBuddy 同样无探测资格。
    await wrapper.setProps({ account: makeAccount({ platform: 'qoder', type: 'apikey' }) })
    expect(wrapper.findAll('button')).toHaveLength(0)
    expect(wrapper.text()).toBe('-')
  })
