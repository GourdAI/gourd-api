...
describe('BulkEditAccountModal', () => {
  ...
  it('WorkBuddy 目标不显示上游倍率自动探测批量开关', () => {
    // WorkBuddy 凭据为 token（无 credentials.api_key），后端已将其移出探测资格
    // 名单：若仍显示开关，提交该字段会被后端以资格不符拒绝，导致整批更新失败。
    const wrapper = mountModal({
      selectedPlatforms: ['workbuddy'],
      selectedTypes: ['apikey']
    })
  ...
  it('WorkBuddy 与其他 API Key 平台混合选择时也不显示探测批量开关', () => {
    const wrapper = mountModal({
      selectedPlatforms: ['workbuddy', 'openai'],
      selectedTypes: ['apikey']
    })
