...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('submits OpenCode Zen default protocol rules with adaptive endpoints', async () => {
    ...
    await wrapper.get('form#create-account-form input[type="password"]').setValue('tk-opencode-zen')
  ...
  it('submits OpenCode GO endpoints after switching account type', async () => {
    ...
    await wrapper.get('form#create-account-form input[type="password"]').setValue('tk-opencode-go')
  ...
  it('submits adaptive Kimi Coding Plan Responses endpoint', async () => {
    ...
    await wrapper.get('form#create-account-form input[type="password"]').setValue('tk-kimi-coding')
  ...
  it('submits WorkBuddy credentials with the CN realm default endpoint', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'WorkBuddy')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('WorkBuddy account')
    await wrapper.get('[data-testid="workbuddy-access-token"]').setValue('wb-access-token')
  ...
  it('switches WorkBuddy to the global endpoint when the global realm is selected', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'WorkBuddy')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('WorkBuddy global')
    ...
    expect(createAccountMock.mock.calls[0]?.[0]?.credentials).toMatchObject({
      realm: 'global',
      // 国际端点必须带 www（裸域触发上游 301 → POST 被改写为 GET → 404 page not found）。
      base_url: 'https://www.workbuddy.ai',
      access_token: 'wb-access-token'
    })
  })

  it('hides the upstream billing probe toggle and omits the flag for WorkBuddy accounts', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'WorkBuddy')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('WorkBuddy account')
    await wrapper.get('[data-testid="workbuddy-access-token"]').setValue('wb-access-token')

    // WorkBuddy 凭据为 token 无 api_key，后端不具备探测资格：开关隐藏
  ...
  it('requires a WorkBuddy access or refresh token before submitting', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'WorkBuddy')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('WorkBuddy account')
  ...
  it('antigravity upstream 创建默认携带上游倍率探测开关', async () => {
    ...
    await wrapper.get('form#create-account-form input[type="password"]').setValue('tk-upstream')
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()
...
describe('CreateAccountModal TLS 指纹开关（全平台）', () => {
  beforeEach(() => {
    authIsSimpleMode.value = true
    createAccountMock.mockReset().mockResolvedValue({ id: 42, platform: 'openai', type: 'apikey' })
  })

  it('非 Anthropic 平台（API Key）也显示 TLS 开关', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'OpenAI')
    await selectButtonByText(wrapper, 'API Key')

    expect(wrapper.find('[data-testid="create-tls-fingerprint"]').exists()).toBe(true)
  })

  it('非 Anthropic 平台开启后写入 extra，默认不写入', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'OpenAI')
    await selectButtonByText(wrapper, 'API Key')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('openai account')
    ...
    // 默认关闭：不得写入 enable_tls_fingerprint
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()
    expect(createAccountMock).toHaveBeenCalledTimes(1)
    expect(createAccountMock.mock.calls[0]?.[0]?.extra?.enable_tls_fingerprint).toBeUndefined()

    // 开启后：必须写入 extra
    createAccountMock.mockClear()
    const enabled = mountModal()
    await selectButtonByText(enabled, 'OpenAI')
    await selectButtonByText(enabled, 'API Key')
    await enabled.get('form#create-account-form input[type="text"]').setValue('openai account')
    await enabled.get('form#create-account-form input[type="password"]').setValue('test-api-key')
    await enabled.get('[data-testid="create-tls-fingerprint-toggle"]').trigger('click')
    await enabled.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()

    expect(createAccountMock).toHaveBeenCalledTimes(1)
    expect(createAccountMock.mock.calls[0]?.[0]?.extra?.enable_tls_fingerprint).toBe(true)
  })
})

describe('CreateAccountModal Qoder 平台表单', () => {
  beforeEach(() => {
    authIsSimpleMode.value = true
    createAccountMock.mockReset().mockResolvedValue({ id: 42, platform: 'qoder', type: 'apikey' })
  })

  it('选中 Qoder 时不渲染国产协议端点编辑器，仅展示 Qoder 专属字段', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'Qoder')

    // 回归：曾因 v-else 兜底误渲染「协议端点」编辑器（CN 多协议平台专用）
    ...
    expect(wrapper.get<HTMLInputElement>('[data-testid="qoder-base-url"]').element.value).toBe(
  ...
  it('国产多协议平台仍渲染协议端点编辑器（对照保护）', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'Kimi')
  ...
  it('提交 Qoder 账号：携带 realm 与 base_url，且不写入 CN 多协议字段', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'Qoder')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('Qoder account')
    await wrapper.get('[data-testid="qoder-access-token"]').setValue('dt-test-token')
    ...
    expect(payload?.credentials?.api_base_urls).toBeUndefined()
    expect(payload?.credentials?.api_protocol).toBeUndefined()
    expect(payload?.credentials?.api_key).toBeUndefined()
  })

  it('切换 realm 到 global 时 base url 联动为 api1.qoder.sh', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'Qoder')
