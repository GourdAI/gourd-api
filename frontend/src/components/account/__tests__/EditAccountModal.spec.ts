...
function buildVertexAccount() {
  return {
    ...
    credentials: {
      service_account_json: '{"type":"service_account","client_email":"sa@example.iam.gserviceaccount.com","private_key":"PEM-TEST-PLACEHOLDER-MIIE-END-PLACEHOLDER"}',
...
describe('EditAccountModal', () => {
  ...
  it('preserves WorkBuddy tokens and submits realm, uid, and enterprise id on edit', async () => {
    const account = buildAccount()
  ...
  it('hides the upstream billing probe toggle and omits the flag for WorkBuddy accounts', async () => {
    // WorkBuddy 凭据为 token 无 api_key，后端不具备探测资格：
    // 编辑页隐藏开关，提交 payload 也不携带探测字段。
    const account = buildAccount()
    account.platform = 'workbuddy'
    account.name = 'wb-relay'
    account.credentials = {
      access_token: 'at',
      refresh_token: 'rt',
      base_url: 'https://copilot.tencent.com'
...
describe('EditAccountModal TLS 指纹开关（全平台）', () => {
  beforeEach(() => {
    authIsSimpleMode.value = true
    updateAccountMock.mockReset()
    checkMixedChannelRiskMock.mockReset().mockResolvedValue({ has_risk: false })
  ...
  it('非 Anthropic 平台也显示 TLS 开关，并回填已保存的开关与模板', () => {
    for (const platform of ['openai', 'gemini', 'grok', 'antigravity', 'workbuddy'] as const) {
      const account = buildAccount()
      account.platform = platform
      // 回填源是后端 DTO 顶层字段（由 extra 提取而来），不是 extra 本体
      account.enable_tls_fingerprint = true
      account.tls_fingerprint_profile_id = 7
      updateAccountMock.mockResolvedValue(account)

      const wrapper = mountModal(account)
      expect(wrapper.find('[data-testid="edit-tls-fingerprint"]').exists()).toBe(true)
      expect(wrapper.get('[data-testid="edit-tls-fingerprint-toggle"]').classes()).toContain('bg-primary-600')
      wrapper.unmount()
    }
  })

  it('非 Anthropic 平台开启后写入 extra，关闭时清理相关键', async () => {
    const account = buildAccount()
    account.platform = 'gemini'
    updateAccountMock.mockResolvedValue(account)

    const wrapper = mountModal(account)
    await wrapper.get('[data-testid="edit-tls-fingerprint-toggle"]').trigger('click')
    await wrapper.get('form#edit-account-form').trigger('submit.prevent')
    ...
    updateAccountMock.mockClear()
    const enabled = buildAccount()
    enabled.platform = 'gemini'
    enabled.enable_tls_fingerprint = true
    enabled.tls_fingerprint_profile_id = 9
    updateAccountMock.mockResolvedValue(enabled)

    const closed = mountModal(enabled)
    await closed.get('[data-testid="edit-tls-fingerprint-toggle"]').trigger('click')
    await closed.get('form#edit-account-form').trigger('submit.prevent')
  ...
  it('未改动 TLS 时不回写 extra，避免覆盖其他运行态键', async () => {
    const account = buildAccount()
    account.platform = 'grok'
    account.extra = { grok_client_tool_cache_enabled: true }
    updateAccountMock.mockResolvedValue(account)
