...
// 探测资格与后端 IsUpstreamBillingProbeIdentity 共用同一判定：WorkBuddy 凭据为
// token（无 credentials.api_key），不具备探测资格——若仍展示手动探测按钮，
// 点击必被后端以 400 拒绝，故整列（含手动探测按钮）一并隐藏。
const eligible = computed(() => isUpstreamBillingProbeCapable(props.account.platform, props.account.type))
