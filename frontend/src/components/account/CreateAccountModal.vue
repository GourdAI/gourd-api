    ...
    >
      ...
      <div>
        ...
        <div class="mt-2 flex flex-wrap rounded-lg bg-gray-100 p-1 dark:bg-dark-700">
          ...
          >
            <PlatformIcon platform="trae" size="sm" />
            Trae
          </button>
        </div>
      </div>
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        <!-- 非 Qoder：普通 base url 输入（多协议 adaptive 平台走下方分协议端点编辑器） -->
        <div v-if="form.platform !== 'qoder' && (!isMultiProtocolPlatform || apiProtocol !== 'adaptive')">
        ...
        <!-- Qoder：base url 由下方 Qoder 专属区块（realm 联动）承载，此处不重复渲染 -->
        <div v-else-if="form.platform !== 'qoder'">
        ...
        />
        <!-- WorkBuddy：双域 APIKey 凭据（access_token / refresh_token 至少填一个） -->
        <template v-if="form.platform === 'workbuddy'">
          <div>
            <label class="input-label">{{ t('admin.accounts.workbuddy.realm.label') }}</label>
            <select v-model="workbuddyRealm" class="input" data-testid="workbuddy-realm">
              <option v-for="opt in WORKBUDDY_REALM_OPTIONS" :key="opt.value" :value="opt.value">
                {{ t(`admin.accounts.workbuddy.realm.${opt.labelKey}`) }}
              </option>
            </select>
            <p class="input-hint">{{ t('admin.accounts.workbuddy.realm.hint') }}</p>
          </div>
          <!-- WorkBuddy OAuth 设备授权登录：生成授权链接 → 浏览器登录 → 自动轮询回填凭据 -->
          <div
            class="rounded-lg border border-violet-200 bg-violet-50 p-4 dark:border-violet-700 dark:bg-violet-900/20"
          >
            <div class="flex items-start gap-3">
              <div
                class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-lg bg-violet-500"
              >
                <Icon name="link" size="sm" class="text-white" />
              </div>
              <div class="flex-1">
                <h4 class="font-medium text-violet-900 dark:text-violet-200">
                  {{ t('admin.accounts.workbuddy.oauthLogin') }}
                </h4>
                <p class="mt-1 text-xs text-violet-700 dark:text-violet-300">
                  {{ t('admin.accounts.workbuddy.oauthLoginHint') }}
                </p>

                <button
                  v-if="!workbuddyOAuthUrl"
                  type="button"
                  class="btn btn-primary mt-3 text-sm"
                  data-testid="workbuddy-oauth-login"
                  :disabled="workbuddyOAuth.loading.value"
                  @click="handleWorkBuddyOAuthLogin"
                >
                  <svg
                    v-if="workbuddyOAuth.loading.value"
                  ...
                  {{
                    workbuddyOAuth.loading.value
                      ? t('admin.accounts.workbuddy.oauthGenerating')
                      : t('admin.accounts.workbuddy.oauthLogin')
                  }}
                </button>
                ...
                <div v-else class="mt-3 space-y-2">
                  <div class="flex items-center gap-2">
                    <input
                      :value="workbuddyOAuthUrl"
                      readonly
                      type="text"
                      class="input flex-1 bg-white font-mono text-xs dark:bg-dark-700"
                      data-testid="workbuddy-oauth-url"
                    />
                    <button
                      type="button"
                      class="btn btn-secondary text-sm"
                      data-testid="workbuddy-oauth-copy"
                      @click="copyWorkBuddyAuthUrl"
                    >
                      {{ t('admin.accounts.workbuddy.oauthCopy') }}
                    </button>
                  </div>
                  <p class="text-xs text-violet-700 dark:text-violet-300">
                    {{ t('admin.accounts.workbuddy.oauthOpenHint') }}
                  </p>
                  <p
                    v-if="workbuddyOAuth.polling.value"
                    class="flex items-center gap-2 text-xs text-violet-700 dark:text-violet-300"
                    data-testid="workbuddy-oauth-polling"
                  >
                    <span
                      class="inline-block h-2 w-2 animate-pulse rounded-full bg-violet-500"
                    ></span>
                    {{ t('admin.accounts.workbuddy.oauthWaiting') }}
                  </p>
                  <p
                    v-if="workbuddyOAuthMessage"
                    class="text-xs"
                    :class="
                      workbuddyOAuthSucceeded
                        ? 'text-green-600 dark:text-green-400'
                        : 'text-amber-600 dark:text-amber-400'
                    "
                    data-testid="workbuddy-oauth-status"
                  >
                    {{ workbuddyOAuthMessage }}
                  </p>
                  <button
                    v-if="!workbuddyOAuth.polling.value && !workbuddyOAuthSucceeded"
                    type="button"
                    class="text-xs text-violet-600 underline hover:text-violet-700 dark:text-violet-400"
                    data-testid="workbuddy-oauth-retry"
                    @click="handleWorkBuddyOAuthLogin"
                  >
                    {{ t('admin.accounts.workbuddy.oauthRetry') }}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div>
            <label class="input-label">{{ t('admin.accounts.workbuddy.accessToken') }}</label>
            <input
              v-model="workbuddyAccessToken"
              type="password"
              class="input font-mono"
              data-testid="workbuddy-access-token"
              :placeholder="t('admin.accounts.workbuddy.tokenPlaceholder')"
            />
          </div>
          <div>
            <label class="input-label">{{ t('admin.accounts.workbuddy.refreshToken') }}</label>
            <input
              v-model="workbuddyRefreshToken"
              type="password"
              class="input font-mono"
              data-testid="workbuddy-refresh-token"
              :placeholder="t('admin.accounts.workbuddy.tokenPlaceholder')"
            />
          </div>
          <p class="input-hint">{{ t('admin.accounts.workbuddy.tokensHint') }}</p>
          <div>
            <label class="input-label">{{ t('admin.accounts.workbuddy.uid') }}</label>
            <input
              v-model="workbuddyUid"
              type="text"
              class="input font-mono"
              data-testid="workbuddy-uid"
              :placeholder="t('admin.accounts.workbuddy.optionalPlaceholder')"
            />
          </div>
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              <label class="input-label">{{ t('admin.accounts.workbuddy.enterpriseId') }}</label>
              <input
                v-model="workbuddyEnterpriseId"
                type="text"
                class="input font-mono"
                data-testid="workbuddy-enterprise-id"
                :placeholder="t('admin.accounts.workbuddy.optionalPlaceholder')"
              />
            </div>
            <div>
              <label class="input-label">{{ t('admin.accounts.workbuddy.deviceToken') }}</label>
              <input
                v-model="workbuddyDeviceToken"
                type="password"
                class="input font-mono"
                :placeholder="t('admin.accounts.workbuddy.optionalPlaceholder')"
              />
        ...
        <!-- Qoder：双域 chat 网关凭据（access_token 必填，支持 dt-/drt-/pt- 直填） -->
        <template v-if="form.platform === 'qoder'">
          <div>
            <label class="input-label">{{ t('admin.accounts.qoder.realm.label') }}</label>
            <select v-model="qoderRealm" class="input" data-testid="qoder-realm">
              <option v-for="opt in QODER_REALM_OPTIONS" :key="opt.value" :value="opt.value">
                {{ t(`admin.accounts.qoder.realm.${opt.labelKey}`) }}
              </option>
            </select>
            <p class="input-hint">{{ t('admin.accounts.qoder.realm.hint') }}</p>
          </div>
          <div>
            <label class="input-label">{{ t('admin.accounts.qoder.baseUrlLabel') }}</label>
            <input v-model="apiKeyBaseUrl" type="text" class="input" data-testid="qoder-base-url" />
            <p class="input-hint">{{ t('admin.accounts.qoder.baseUrlHint') }}</p>
          </div>
          <!-- Qoder OAuth 设备授权登录：生成登录链接 → 浏览器登录 → 自动轮询回填凭据 -->
          <div
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <h4 class="font-medium text-purple-900 dark:text-purple-200">
                  {{ t('admin.accounts.qoder.oauthLogin') }}
                </h4>
                <p class="mt-1 text-xs text-purple-700 dark:text-purple-300">
                  {{ t('admin.accounts.qoder.oauthHowTo') }}
                </p>
                ...
                <button
                  ...
                  type="button"
                  class="btn btn-primary mt-3 text-sm"
                  data-testid="qoder-oauth-login"
                  :disabled="qoderOAuth.loading.value"
                  @click="handleQoderOAuthLogin"
                >
                  <svg
                    v-if="qoderOAuth.loading.value"
                    class="-ml-1 mr-2 h-4 w-4 animate-spin"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
                  ...
                  {{
                    qoderOAuth.loading.value
                      ? t('admin.accounts.qoder.oauthPending')
                      : t('admin.accounts.qoder.oauthLogin')
                  }}
                </button>
                ...
                <div v-else class="mt-3 space-y-2">
                  <div class="flex items-center gap-2">
                    ...
                    <a
                      ...
                      class="btn btn-secondary text-sm"
                      data-testid="qoder-oauth-open"
                    >
                      {{ t('admin.accounts.qoder.oauthLogin') }}
                    </a>
                  </div>
                  <p
                    v-if="qoderOAuth.polling.value"
                    class="flex items-center gap-2 text-xs text-purple-700 dark:text-purple-300"
                    data-testid="qoder-oauth-polling"
                  >
                    <span
                      class="inline-block h-2 w-2 animate-pulse rounded-full bg-purple-500"
                    ></span>
                    {{ t('admin.accounts.qoder.oauthPending') }}
                  </p>
                  <p
                    v-if="qoderOAuthMessage"
                    class="text-xs"
                    :class="
                      qoderOAuthSucceeded
                        ? 'text-green-600 dark:text-green-400'
                        : 'text-amber-600 dark:text-amber-400'
                    "
                    data-testid="qoder-oauth-status"
                  >
                    {{ qoderOAuthMessage }}
                  </p>
                  <button
                    v-if="!qoderOAuth.polling.value && !qoderOAuthSucceeded"
                    type="button"
                    class="text-xs text-purple-600 underline hover:text-purple-700 dark:text-purple-400"
                    data-testid="qoder-oauth-retry"
                    @click="handleQoderOAuthLogin"
                  >
                    {{ t('admin.accounts.qoder.oauthTimeout') }}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div>
            <label class="input-label">{{ t('admin.accounts.qoder.accessTokenLabel') }}</label>
            <input
              v-model="qoderAccessToken"
              type="password"
              class="input font-mono"
              data-testid="qoder-access-token"
            />
            <p class="input-hint">{{ t('admin.accounts.qoder.accessTokenHint') }}</p>
            <p class="input-hint">{{ t('admin.accounts.qoder.patHint') }}</p>
          </div>
          <div>
            <label class="input-label">{{ t('admin.accounts.qoder.refreshTokenLabel') }}</label>
            <input
              v-model="qoderRefreshToken"
              type="password"
              class="input font-mono"
              data-testid="qoder-refresh-token"
            />
            <p class="input-hint">{{ t('admin.accounts.qoder.refreshTokenHint') }}</p>
          </div>
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              <label class="input-label">{{ t('admin.accounts.qoder.uidLabel') }}</label>
              <input
                v-model="qoderUid"
                type="text"
                class="input font-mono"
                data-testid="qoder-uid"
              />
              <p class="input-hint">{{ t('admin.accounts.qoder.uidHint') }}</p>
            </div>
            <div>
              <label class="input-label">{{ t('admin.accounts.qoder.nicknameLabel') }}</label>
              <input
                v-model="qoderNickname"
                type="text"
                class="input font-mono"
                data-testid="qoder-nickname"
              />
              <p class="input-hint">{{ t('admin.accounts.qoder.nicknameHint') }}</p>
            </div>
          </div>
        </template>
        <!-- Trae：双域固定协议凭据（access_token / refresh_token 至少填一个；留空的密钥字段沿用现有值） -->
        <template v-else-if="form.platform === 'trae'">
          <div>
            <label class="input-label">{{ t('admin.accounts.trae.realm.label') }}</label>
            <select v-model="traeRealm" class="input" data-testid="trae-realm">
                ...
                <div v-else class="mt-3 space-y-2">
                  ...
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <span
                      ...
                      data-testid="trae-oauth-countdown"
                    >
                  ...
                  <textarea
                    ...
                    data-testid="trae-oauth-callback"
                  ...
                  <div class="flex items-center gap-2">
                    <button
                      ...
                      data-testid="trae-oauth-submit"
                      :disabled="!traeCallbackUrl.trim() || traeOAuth.submitting.value || traeOAuth.expired.value"
          ...
          <div>
            ...
            <textarea
              v-model="traeAccessToken"
              rows="3"
              class="input font-mono"
              data-testid="trae-access-token"
          ...
          <div>
            ...
            <div class="flex items-start gap-2">
              <input
                v-model="traeRefreshToken"
                type="password"
                class="input flex-1 font-mono min-w-0"
                data-testid="trae-refresh-token"
                :placeholder="t('admin.accounts.trae.tokenPlaceholder')"
              />
              ...
              <button
                ...
                data-testid="trae-exchange-token"
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                v-model="traeDeviceId"
                type="text"
                class="input font-mono"
                data-testid="trae-device-id"
                :placeholder="t('admin.accounts.trae.optionalPlaceholder')"
              />
            </div>
            <div>
              ...
              <input
                v-model="traeMachineId"
                type="text"
                class="input font-mono"
                data-testid="trae-machine-id"
                :placeholder="t('admin.accounts.trae.optionalPlaceholder')"
              />
            </div>
            <div>
              ...
              <input
                v-model="traeIdeVersionCode"
                type="text"
                class="input font-mono"
                data-testid="trae-ide-version-code"
                :placeholder="t('admin.accounts.trae.optionalPlaceholder')"
              />
          ...
          <div>
            ...
            <input
              v-model="traeBillingBaseUrl"
              type="text"
              class="input font-mono"
              data-testid="trae-billing-base-url"
              :placeholder="t('admin.accounts.trae.optionalPlaceholder')"
            />
            <p class="input-hint">{{ t('admin.accounts.trae.billingBaseUrlHint') }}</p>
          </div>
        </template>
        <div v-else-if="form.platform !== 'workbuddy'">
        ...
        <!-- 上游倍率自动探测：全部 API-key 平台可用（所在区块已限定 apikey 类型）；
             WorkBuddy / Qoder / Trae 凭据为 token 无 api_key，后端不具备探测资格，隐藏开关 -->
        <div
          v-if="form.platform !== 'workbuddy' && form.platform !== 'qoder' && form.platform !== 'trae'"
      ...
      <!-- TLS Fingerprint（全平台有效，防封加固） -->
      <div
        data-testid="create-tls-fingerprint"
        class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4"
      >
        <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          <div class="flex items-center justify-between">
...
const withUpstreamRequestIdHeader = <T extends Record<string, unknown> | undefined>(extra: T): Record<string, unknown> | undefined => {
  const next: Record<string, unknown> = { ...(extra || {}) }
  const name = upstreamRequestIdHeader.value.trim()
  if (name) {
    next.upstream_request_id_header = name
  }
  // TLS 指纹伪装（全平台有效，防封加固）：所有创建链路都经过本 helper，
  // 在此统一注入，覆盖 apikey / oauth / 批量导入等全部路径。
  if (tlsFingerprintEnabled.value) {
    next.enable_tls_fingerprint = true
    if (tlsFingerprintProfileId.value) {
      next.tls_fingerprint_profile_id = tlsFingerprintProfileId.value
    } else {
      delete next.tls_fingerprint_profile_id
    }
  } else {
    delete next.enable_tls_fingerprint
    delete next.tls_fingerprint_profile_id
  }
...
const baseUrlHint = computed(() => {
  ...
  if (form.platform === 'qoder') return t('admin.accounts.qoder.baseUrlHint')
  if (form.platform === 'trae') return t('admin.accounts.trae.baseUrlHint')
...
const apiKeyBaseUrlPlaceholder = computed(() => {
  ...
  switch (form.platform) {
    ...
    case 'qoder':
      return 'https://gateway.qoder.com.cn'
    case 'trae':
...
const workbuddyOAuth = useWorkBuddyOAuth() // For WorkBuddy device OAuth
const qoderOAuth = useQoderOAuth() // For Qoder device OAuth
...
// ── WorkBuddy（双域 APIKey 网关）凭据字段 ──
const workbuddyAccessToken = ref('')
const workbuddyRefreshToken = ref('')
const workbuddyRealm = ref<WorkBuddyRealm>('cn')
const workbuddyUid = ref('')
const workbuddyEnterpriseId = ref('')
const workbuddyDeviceToken = ref('')
// ── WorkBuddy OAuth 设备授权登录 ──
const workbuddyOAuthUrl = computed(() => workbuddyOAuth.authUrl.value)
const workbuddyOAuthMessage = ref('')
const workbuddyOAuthSucceeded = ref(false)
// ── Qoder（双域 chat 网关）凭据字段 ──
const qoderAccessToken = ref('')
const qoderRefreshToken = ref('')
const qoderRealm = ref<QoderRealm>('cn')
const qoderUid = ref('')
const qoderNickname = ref('')
// ── Qoder OAuth 设备授权登录 ──
const qoderOAuthUrl = computed(() => qoderOAuth.loginUrl.value)
const qoderOAuthMessage = ref('')
const qoderOAuthSucceeded = ref(false)
// ── Trae（字节 Trae IDE 代理）凭据字段 ──
const traeAccessToken = ref('')
const traeRefreshToken = ref('')
const traeRealm = ref<TraeRealm>('cn')
const traeUid = ref('')
const traeDeviceId = ref('')
const traeMachineId = ref('')
const traeBillingBaseUrl = ref('')
const traeIdeVersionCode = ref('')
// 凭据到期时刻（epoch 秒）：由换票响应回填，或用户手动粘贴 storage.json 的到期时间。
const traeExpiresAt = ref<number | null>(null)
const traeRefreshExpiresAt = ref<number | null>(null)
// ── Trae 换票（refresh_token → 新 access_token / 轮换后的 refresh_token）──
const traeExchangeLoading = ref(false)
// 切换 Trae：强制 apikey 类型，区域回落 cn，base url 重置为对应域默认端点。
function selectTraePlatform() {
  form.platform = 'trae'
  form.type = 'apikey'
  accountCategory.value = 'apikey'
  traeRealm.value = 'cn'
  apiKeyBaseUrl.value = defaultTraeBaseUrl('cn')
}
// Trae 换票：成功则回填 access_token / refresh_token（后者必须覆盖旧值）与 uid。
const handleTraeExchangeToken = async () => {
  if (traeExchangeLoading.value) return
  const refreshToken = traeRefreshToken.value.trim()
  if (!refreshToken) {
  ...
  try {
    ...
    if (result.access_token) traeAccessToken.value = String(result.access_token)
    // 轮换后的 refresh_token 必须覆盖旧值（旧值已失效）。
    if (result.refresh_token) traeRefreshToken.value = String(result.refresh_token)
    if (result.uid) traeUid.value = String(result.uid)
    if (result.realm) traeRealm.value = resolveTraeRealm(result.realm)
    // 到期时刻同步回填：不回填则账号凭据里没有 expires_at，管理页的到期告警
    // 与「提前换票」都会失效（refreshToken 过期 = 账号静默不可用）。
    if (typeof result.expires_at === 'number' && result.expires_at > 0) {
      traeExpiresAt.value = result.expires_at
    }
    if (typeof result.refresh_expires_at === 'number' && result.refresh_expires_at > 0) {
      traeRefreshExpiresAt.value = result.refresh_expires_at
    }
...
// 不在此列表里的键一律当 extras 透传入库，避免丢掉上游设备信息。
...
const applyTraeOAuthCredentials = (
  ...
  if (next.access_token) traeAccessToken.value = String(next.access_token)
  // 浏览器登录会签发全新的令牌对，refresh_token 直接覆盖旧值（与换票入口同口径）。
  if (next.refresh_token) traeRefreshToken.value = String(next.refresh_token)
  if (next.uid) traeUid.value = String(next.uid)
  if (next.device_id) traeDeviceId.value = String(next.device_id)
  if (next.machine_id) traeMachineId.value = String(next.machine_id)
  if (next.realm) traeRealm.value = resolveTraeRealm(next.realm)
  ...
  // 到期时刻必须回填：否则凭据里没有 expires_at，列表页的到期告警与提前换票都会静默失效。
  const expiresAt = readEpoch(next.expires_at)
  if (expiresAt) traeExpiresAt.value = expiresAt
  const refreshExpiresAt = readEpoch(next.refresh_expires_at)
  if (refreshExpiresAt) traeRefreshExpiresAt.value = refreshExpiresAt
  ...
  // 账号名：仅在用户未填时自动回填（后端 account_name 优先，回落凭据里的 nickname）。
...
const handleTraeOAuthLogin = async () => {
  ...
  if (traeOAuthUrl.value) {
    ...
    if (opened) {
      ...
      // 弹窗被浏览器拦截时保留可点击链接与复制按钮，用户仍可手动完成授权。
...
watch(traeRealm, (realm, previousRealm) => {
  if (form.platform !== 'trae') return
  if (!apiKeyBaseUrl.value.trim() || apiKeyBaseUrl.value === defaultTraeBaseUrl(previousRealm)) {
    apiKeyBaseUrl.value = defaultTraeBaseUrl(realm)
  }
})
...
// 切换 WorkBuddy：强制 apikey 类型，区域回落 cn，base url 重置为对应域默认端点。
function selectWorkBuddyPlatform() {
  form.platform = 'workbuddy'
  form.type = 'apikey'
  accountCategory.value = 'apikey'
  workbuddyRealm.value = 'cn'
  apiKeyBaseUrl.value = defaultWorkBuddyBaseUrl('cn')
}
// 切换 Qoder：强制 apikey 类型，区域回落 cn，base url 重置为对应域默认端点。
function selectQoderPlatform() {
  form.platform = 'qoder'
  form.type = 'apikey'
  accountCategory.value = 'apikey'
  qoderRealm.value = 'cn'
  apiKeyBaseUrl.value = defaultQoderBaseUrl('cn')
}
// WorkBuddy OAuth：生成授权链接并启动轮询；realm 变化时重新发起。
const resetWorkBuddyOAuthState = () => {
  workbuddyOAuth.resetState()
  workbuddyOAuthMessage.value = ''
  workbuddyOAuthSucceeded.value = false
}
const applyWorkBuddyOAuthToken = (token: WorkBuddyTokenInfo) => {
  // 直接赋值回填（OAuth 获取的是完整新凭据，不走「留空=沿用」逻辑）。
  if (token.access_token) workbuddyAccessToken.value = String(token.access_token)
  if (token.refresh_token) workbuddyRefreshToken.value = String(token.refresh_token)
  if (token.uid) workbuddyUid.value = String(token.uid)
  if (token.enterprise_id) workbuddyEnterpriseId.value = String(token.enterprise_id)
  workbuddyOAuthSucceeded.value = true
  workbuddyOAuthMessage.value = t('admin.accounts.workbuddy.oauthSuccess')
  appStore.showSuccess(workbuddyOAuthMessage.value)
}
const handleWorkBuddyOAuthLogin = async () => {
  resetWorkBuddyOAuthState()
  const ok = await workbuddyOAuth.generateAuthUrl(workbuddyRealm.value, form.proxy_id)
  if (!ok) return
  workbuddyOAuth.startPolling(
    {
      onCompleted: (token) => applyWorkBuddyOAuthToken(token),
      onTimeout: () => {
        workbuddyOAuthMessage.value = t('admin.accounts.workbuddy.oauthTimeout')
      },
      onError: () => {
        // 单次失败容忍，仅在界面上提示仍在等待。
        workbuddyOAuthMessage.value = t('admin.accounts.workbuddy.oauthWaiting')
      },
    },
    { realm: workbuddyRealm.value, proxyId: form.proxy_id }
  )
}
const { copyToClipboard } = useClipboard()
const copyWorkBuddyAuthUrl = () => {
  if (workbuddyOAuthUrl.value) {
    copyToClipboard(
      workbuddyOAuthUrl.value,
      t('admin.accounts.workbuddy.oauthCopySuccess')
    )
  }
}
// realm 切换时放弃当前授权链接与轮询（state 与 realm 绑定，避免兑换错域）。
watch(workbuddyRealm, () => {
  if (form.platform !== 'workbuddy') return
  resetWorkBuddyOAuthState()
})
// WorkBuddy realm 变更时同步默认 base url（用户自定义地址不覆盖）。
watch(workbuddyRealm, (realm, previousRealm) => {
  if (form.platform !== 'workbuddy') return
  if (!apiKeyBaseUrl.value.trim() || apiKeyBaseUrl.value === defaultWorkBuddyBaseUrl(previousRealm)) {
    apiKeyBaseUrl.value = defaultWorkBuddyBaseUrl(realm)
  }
})
// Qoder OAuth：生成授权链接并启动轮询；realm 变化时重新发起。
const resetQoderOAuthState = () => {
  qoderOAuth.resetState()
  qoderOAuthMessage.value = ''
  qoderOAuthSucceeded.value = false
}
const applyQoderOAuthCredentials = (credentials: Record<string, string>) => {
  // 直接赋值回填（OAuth 获取的是完整新凭据，不走「留空=沿用」逻辑）。
  if (credentials.access_token) qoderAccessToken.value = String(credentials.access_token)
  if (credentials.refresh_token) qoderRefreshToken.value = String(credentials.refresh_token)
  if (credentials.uid) qoderUid.value = String(credentials.uid)
  qoderOAuthSucceeded.value = true
  qoderOAuthMessage.value = t('admin.accounts.qoder.oauthSuccess')
  appStore.showSuccess(qoderOAuthMessage.value)
}
const handleQoderOAuthLogin = async () => {
  resetQoderOAuthState()
  const ok = await qoderOAuth.generateAuthUrl(qoderRealm.value)
  if (!ok) return
  // 打开新窗口完成 qoder.com 授权页登录（授权 URL 与 login_id 由后端签发）。
  if (qoderOAuthUrl.value) {
    window.open(qoderOAuthUrl.value, '_blank', 'noopener,noreferrer')
  }
  qoderOAuth.startPolling(
    {
      onCompleted: (credentials) => applyQoderOAuthCredentials(credentials),
      onTimeout: () => {
        qoderOAuthMessage.value = t('admin.accounts.qoder.oauthTimeout')
      },
      onError: () => {
        // 单次失败容忍，仅在界面上提示仍在等待。
        qoderOAuthMessage.value = t('admin.accounts.qoder.oauthPending')
      },
    },
    { realm: qoderRealm.value }
  )
}
// Qoder realm 切换时放弃当前授权链接与轮询（login_id 与 realm 绑定，避免兑换错域）。
watch(qoderRealm, () => {
  if (form.platform !== 'qoder') return
  resetQoderOAuthState()
})
// Qoder realm 变更时同步默认 base url（用户自定义地址不覆盖）。
watch(qoderRealm, (realm, previousRealm) => {
  if (form.platform !== 'qoder') return
  if (!apiKeyBaseUrl.value.trim() || apiKeyBaseUrl.value === defaultQoderBaseUrl(previousRealm)) {
    apiKeyBaseUrl.value = defaultQoderBaseUrl(realm)
  }
})
...
watch(
  ...
  (newPlatform) => {
    ...
    if (newPlatform === 'workbuddy') {
      apiKeyBaseUrl.value = defaultWorkBuddyBaseUrl(workbuddyRealm.value)
    } else if (newPlatform === 'qoder') {
      apiKeyBaseUrl.value = defaultQoderBaseUrl(qoderRealm.value)
    } else if (newPlatform === 'trae') {
      apiKeyBaseUrl.value = defaultTraeBaseUrl(traeRealm.value)
    ...
    // Reset WorkBuddy fields when switching away
    if (newPlatform !== 'workbuddy') {
      workbuddyAccessToken.value = ''
      workbuddyRefreshToken.value = ''
      workbuddyRealm.value = 'cn'
      workbuddyUid.value = ''
      workbuddyEnterpriseId.value = ''
      workbuddyDeviceToken.value = ''
      // 切换离开 workbuddy 时停止 OAuth 轮询（state 已失效）。
      resetWorkBuddyOAuthState()
    }
    // Reset Qoder fields when switching away
    if (newPlatform !== 'qoder') {
      ...
      qoderUid.value = ''
      qoderNickname.value = ''
      // 切换离开 qoder 时停止 OAuth 轮询（login_id 已失效）。
      resetQoderOAuthState()
    }
    // 切换离开 trae 时放弃浏览器登录会话（login_id 已因平台切换而失效）。
    if (newPlatform !== 'trae') {
      void traeOAuth.cancelLogin()
      resetTraeOAuthState()
    }
    // Reset Trae fields when switching away
    if (newPlatform !== 'trae') {
      traeAccessToken.value = ''
      traeRefreshToken.value = ''
      traeRealm.value = 'cn'
      traeUid.value = ''
      traeDeviceId.value = ''
      traeMachineId.value = ''
      traeBillingBaseUrl.value = ''
      traeIdeVersionCode.value = ''
    }
...
const resetForm = () => {
  ...
  traeBillingBaseUrl.value = ''
  traeIdeVersionCode.value = ''
  traeExpiresAt.value = null
  traeRefreshExpiresAt.value = null
  // 同时清掉浏览器登录态（loginUrl/粘贴框/extras）：残留会让下次打开显示旧授权
  // 链接与「已成功」提示，且旧账号的 device 凭据会被合并进新建账号。
  resetTraeOAuthState()
...
const handleClose = () => {
  antigravityMixedChannelConfirmed.value = false
  clearMixedChannelDialog()
  // 关闭弹窗时停止 WorkBuddy OAuth 轮询，避免遗留定时器。
  workbuddyOAuth.stopPolling()
  // 关闭弹窗时停止 Qoder OAuth 轮询，避免遗留定时器。
...
// 组件卸载时清理 WorkBuddy / Qoder OAuth 轮询定时器。
onBeforeUnmount(() => {
  workbuddyOAuth.stopPolling()
  qoderOAuth.stopPolling()
...
const handleSubmit = async () => {
  ...
  if (form.platform === 'workbuddy') {
    // WorkBuddy：access_token / refresh_token 至少填一个
    if (!validateWorkbuddyCredentials(workbuddyAccessToken.value, workbuddyRefreshToken.value)) {
      appStore.showError(t('admin.accounts.workbuddy.credentialsRequired'))
      return
    }
  } else if (form.platform === 'qoder') {
    // Qoder：access_token 必填（也接受 dt-/drt-/pt- 直填）
    if (!validateQoderCredentials(qoderAccessToken.value)) {
      appStore.showError(t('admin.accounts.qoder.credentialsRequired'))
      return
    }
  } else if (form.platform === 'trae') {
    // Trae：access_token / refresh_token 至少填一个（与后端 NormalizeTraeCredentials 一致）
    if (!validateTraeCredentials(traeAccessToken.value, traeRefreshToken.value)) {
    ...
    form.platform === 'openai'
      ...
      : form.platform === 'gemini'
        ...
        : form.platform === 'grok'
          ? 'https://api.x.ai/v1'
          : form.platform === 'workbuddy'
            ? defaultWorkBuddyBaseUrl(workbuddyRealm.value)
            : form.platform === 'qoder'
              ? defaultQoderBaseUrl(qoderRealm.value)
              : form.platform === 'trae'
                ? defaultTraeBaseUrl(traeRealm.value)
                : 'https://api.anthropic.com'
  ...
  if (form.platform === 'workbuddy') {
    // WorkBuddy：双域 APIKey 凭据（snake_case，与后端约定一致）
    Object.assign(
      credentials,
      buildWorkbuddyCredentials({
        accessToken: workbuddyAccessToken.value,
        refreshToken: workbuddyRefreshToken.value,
        realm: workbuddyRealm.value,
        uid: workbuddyUid.value,
        enterpriseId: workbuddyEnterpriseId.value,
        deviceToken: workbuddyDeviceToken.value,
        baseUrl: apiKeyBaseUrl.value.trim() || defaultBaseUrl
      })
    )
  } else if (form.platform === 'qoder') {
    // Qoder：双域 chat 凭据（snake_case，与后端约定一致）
    Object.assign(
      credentials,
      buildQoderCredentials({
        accessToken: qoderAccessToken.value,
        refreshToken: qoderRefreshToken.value,
        expiresAt: '',
        deviceToken: '',
        uid: qoderUid.value,
        realm: qoderRealm.value,
        baseUrl: apiKeyBaseUrl.value.trim() || defaultBaseUrl,
        nickname: qoderNickname.value
      })
    )
  } else if (form.platform === 'trae') {
    ...
    Object.assign(
      ...
      traeOAuthExtraCredentials.value,
      buildTraeCredentials({
        accessToken: traeAccessToken.value,
        refreshToken: traeRefreshToken.value,
        realm: traeRealm.value,
        uid: traeUid.value,
        deviceId: traeDeviceId.value,
        machineId: traeMachineId.value,
        baseUrl: apiKeyBaseUrl.value.trim() || defaultBaseUrl,
        billingBaseUrl: traeBillingBaseUrl.value,
        ideVersionCode: traeIdeVersionCode.value,
        expiresAt: traeExpiresAt.value,
        refreshExpiresAt: traeRefreshExpiresAt.value
      })
    )
  } else {
  ...
  await doCreateAccount({
    ...
    // 探测资格与后端 IsUpstreamBillingProbeIdentity 共用同一判定；
    // WorkBuddy 等不合格平台不传该字段，避免后端以 probe 资格不符拒绝创建。
    upstream_billing_probe_enabled:
      isUpstreamBillingProbeCapable(form.platform, 'apikey')
...
const createAccountAndFinish = async (
  ...
  await doCreateAccount({
    ...
    // 上游倍率探测对全部有资格的 API-key 平台开放（antigravity upstream 走本 helper）；
    // 非 apikey 类型（bedrock/oauth）不传，后端不动作；
    // WorkBuddy 等无探测资格平台同样不传（共用后端资格判定）。
    upstream_billing_probe_enabled: isUpstreamBillingProbeCapable(platform, type)
      ? upstreamBillingAutoProbeEnabled.value
      : undefined,
    auto_pause_on_expired: autoPauseOnExpired.value
  })
