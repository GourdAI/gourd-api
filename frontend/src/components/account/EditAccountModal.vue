  ...
  >
    ...
    >
      ...
      <div v-if="account.type === 'apikey'" class="space-y-4">
        ...
        <!-- WorkBuddy：双域 APIKey 凭据（留空的密钥字段沿用现有值） -->
        <template v-if="account.platform === 'workbuddy'">
          <div>
            <label class="input-label">{{ t('admin.accounts.workbuddy.realm.label') }}</label>
            <select v-model="editWorkbuddyRealm" class="input" data-testid="edit-workbuddy-realm">
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                <button
                  ...
                  data-testid="edit-workbuddy-oauth-login"
                ...
                <div v-else class="mt-3 space-y-2">
                  <div class="flex items-center gap-2">
                    <input
                      ...
                      data-testid="edit-workbuddy-oauth-url"
                    />
                    <button
                      type="button"
                      class="btn btn-secondary text-sm"
                      data-testid="edit-workbuddy-oauth-copy"
                  ...
                  <p
                    ...
                    data-testid="edit-workbuddy-oauth-polling"
                  >
                    <span
                  ...
                  <p
                    ...
                    "
                    data-testid="edit-workbuddy-oauth-status"
                  >
                  ...
                  <button
                    ...
                    data-testid="edit-workbuddy-oauth-retry"
                    @click="handleWorkBuddyOAuthLogin"
                  >
          ...
          <div>
            ...
            <input
              v-model="editWorkbuddyAccessToken"
              type="password"
          ...
          <div>
            ...
            <input
              v-model="editWorkbuddyRefreshToken"
              type="password"
              class="input font-mono"
              autocomplete="new-password"
              data-1p-ignore
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                v-model="editWorkbuddyEnterpriseId"
                type="text"
                class="input font-mono"
                :placeholder="t('admin.accounts.workbuddy.optionalPlaceholder')"
              />
            ...
            <div>
              ...
              <input
                v-model="editWorkbuddyDeviceToken"
                type="password"
                class="input font-mono"
                autocomplete="new-password"
                data-1p-ignore
        ...
        <!-- Qoder：双域 chat 网关凭据（留空的密钥字段沿用现有值） -->
        <template v-if="account.platform === 'qoder'">
          <div>
            <label class="input-label">{{ t('admin.accounts.qoder.realm.label') }}</label>
            <select v-model="editQoderRealm" class="input" data-testid="edit-qoder-realm">
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                ...
                <button
                  ...
                  type="button"
                  class="btn btn-primary mt-3 text-sm"
                  data-testid="edit-qoder-oauth-login"
                ...
                <div v-else class="mt-3 space-y-2">
                  <div class="flex items-center gap-2">
                    <input
                      ...
                      type="text"
                      class="input flex-1 bg-white font-mono text-xs dark:bg-dark-700"
                      data-testid="edit-qoder-oauth-url"
                    />
                    <a
                      :href="qoderOAuthUrl"
                      target="_blank"
                      ...
                      class="btn btn-secondary text-sm"
                      data-testid="edit-qoder-oauth-open"
                    >
                      {{ t('admin.accounts.qoder.oauthLogin') }}
                    </a>
                  </div>
                  <p
                    ...
                    data-testid="edit-qoder-oauth-polling"
                  >
                    <span
                  ...
                  <p
                    ...
                    :class="
                      qoderOAuthSucceeded
                        ...
                        : 'text-amber-600 dark:text-amber-400'
                    "
                    data-testid="edit-qoder-oauth-status"
                  >
                    {{ qoderOAuthMessage }}
                  </p>
                  <button
                    ...
                    data-testid="edit-qoder-oauth-retry"
                    @click="handleQoderOAuthLogin"
                  >
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                v-model="editQoderUid"
                type="text"
                class="input font-mono"
                :placeholder="t('admin.accounts.qoder.uidHint')"
              />
            </div>
            <div>
              <label class="input-label">{{ t('admin.accounts.qoder.nicknameLabel') }}</label>
              <input
                v-model="editQoderNickname"
                type="text"
                class="input font-mono"
      ...
      <div
        data-testid="edit-tls-fingerprint"
        class="border-t border-gray-200 pt-4 dark:border-dark-600 space-y-4"
      >
        <div class="rounded-lg border border-gray-200 p-4 dark:border-dark-600">
          <div class="flex items-center justify-between">
...
const editWorkbuddyAccessToken = ref('')
const editWorkbuddyRefreshToken = ref('')
const editWorkbuddyRealm = ref<WorkBuddyRealm>('cn')
const editWorkbuddyUid = ref('')
const editWorkbuddyEnterpriseId = ref('')
const editWorkbuddyDeviceToken = ref('')
// ── Qoder（双域 chat 网关）凭据字段 ──
const editQoderAccessToken = ref('')
const editQoderRefreshToken = ref('')
...
const applyWorkBuddyOAuthToken = (token: WorkBuddyTokenInfo) => {
  ...
  if (token.access_token) editWorkbuddyAccessToken.value = String(token.access_token)
  if (token.refresh_token) editWorkbuddyRefreshToken.value = String(token.refresh_token)
  if (token.uid) editWorkbuddyUid.value = String(token.uid)
  if (token.enterprise_id) editWorkbuddyEnterpriseId.value = String(token.enterprise_id)
...
const handleWorkBuddyOAuthLogin = async () => {
  ...
  const ok = await workbuddyOAuth.generateAuthUrl(editWorkbuddyRealm.value, form.proxy_id)
  if (!ok) return
  workbuddyOAuth.startPolling(
    ...
    { realm: editWorkbuddyRealm.value, proxyId: form.proxy_id }
  )
}
...
const applyQoderOAuthCredentials = (credentials: Record<string, string>) => {
  // 直接赋值回填（OAuth 获取的是完整新凭据，不走「留空=沿用」逻辑）。
  if (credentials.access_token) editQoderAccessToken.value = String(credentials.access_token)
  if (credentials.refresh_token) editQoderRefreshToken.value = String(credentials.refresh_token)
  if (credentials.uid) editQoderUid.value = String(credentials.uid)
...
const handleQoderOAuthLogin = async () => {
  ...
  const ok = await qoderOAuth.generateAuthUrl(editQoderRealm.value)
  if (!ok) return
  ...
  qoderOAuth.startPolling(
    ...
    { realm: editQoderRealm.value }
  )
}
// Qoder realm 切换时放弃当前授权链接与轮询（login_id 与 realm 绑定，避免兑换错域）。
watch(editQoderRealm, () => {
  if (props.account?.platform !== 'qoder') return
  resetQoderOAuthState()
})
...
watch(editWorkbuddyRealm, (realm, previousRealm) => {
  if (props.account?.platform !== 'workbuddy' || syncingForm.value) return
  if (!editBaseUrl.value.trim() || editBaseUrl.value === defaultWorkBuddyBaseUrl(previousRealm)) {
    editBaseUrl.value = defaultWorkBuddyBaseUrl(realm)
  }
})
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    // WorkBuddy：回填区域/UID/企业 ID；密钥类字段留空（响应已脱敏，留空=沿用）。
    if (newAccount.platform === 'workbuddy') {
      editWorkbuddyRealm.value = resolveWorkBuddyRealm(credentials.realm)
      editWorkbuddyUid.value = typeof credentials.uid === 'string' ? credentials.uid : ''
      editWorkbuddyEnterpriseId.value =
        typeof credentials.enterprise_id === 'string' ? credentials.enterprise_id : ''
      editWorkbuddyAccessToken.value = ''
      editWorkbuddyRefreshToken.value = ''
      editWorkbuddyDeviceToken.value = ''
    }
    // Qoder：回填区域/UID/昵称；密钥类字段留空（响应已脱敏，留空=沿用）。
    if (newAccount.platform === 'qoder') {
      editQoderRealm.value = resolveQoderRealm(credentials.realm)
      editQoderUid.value = typeof credentials.uid === 'string' ? credentials.uid : ''
      editQoderNickname.value = typeof credentials.nickname === 'string' ? credentials.nickname : ''
  ...
  editApiKey.value = ''
  // 非 WorkBuddy 账号：清空 WorkBuddy 字段，避免切换账号时残留。
  if (newAccount.platform !== 'workbuddy') {
    editWorkbuddyAccessToken.value = ''
    editWorkbuddyRefreshToken.value = ''
    editWorkbuddyRealm.value = 'cn'
    editWorkbuddyUid.value = ''
    editWorkbuddyEnterpriseId.value = ''
    editWorkbuddyDeviceToken.value = ''
  }
  // 非 Qoder 账号：清空 Qoder 字段，避免切换账号时残留。
  if (newAccount.platform !== 'qoder') {
    editQoderAccessToken.value = ''
    editQoderRefreshToken.value = ''
...
// Load quota control settings (Anthropic OAuth/SetupToken only)
// 注意：TLS 指纹开关已改为全平台生效，其回填在平台门控之前完成。
function loadQuotaControlSettings(account: Account) {
  ...
  // TLS 指纹伪装（全平台有效，防封加固）：必须在平台门控 early return 之前回填，
  // 否则非 Anthropic 账号打开弹窗时开关状态无法正确显示。
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      // 探测资格与后端 IsUpstreamBillingProbeIdentity 共用同一判定：不合格平台
      // （如 WorkBuddy，凭据为 token 无 api_key）不提交该字段，避免 400 拒绝更新。
    ...
    if (props.account.type === 'apikey') {
      ...
      if (props.account.platform === 'workbuddy') {
        ...
        // 密钥类字段留空沿用现有值；uid / enterprise_id 清空时显式删除（全量替换语义）。
        const hasExistingWorkbuddyToken =
        ...
        if (
          !editWorkbuddyAccessToken.value.trim() &&
          !editWorkbuddyRefreshToken.value.trim() &&
          !hasExistingWorkbuddyToken
        ) {
        ...
        Object.assign(
          newCredentials,
          buildWorkbuddyCredentials({
            accessToken: editWorkbuddyAccessToken.value,
            refreshToken: editWorkbuddyRefreshToken.value,
            realm: editWorkbuddyRealm.value,
            uid: editWorkbuddyUid.value,
            enterpriseId: editWorkbuddyEnterpriseId.value,
            deviceToken: editWorkbuddyDeviceToken.value,
            baseUrl: newBaseUrl
          })
        )
        if (!editWorkbuddyUid.value.trim()) delete newCredentials.uid
        if (!editWorkbuddyEnterpriseId.value.trim()) delete newCredentials.enterprise_id
      } else if (props.account.platform === 'qoder') {
        // Qoder：双域 chat 凭据（snake_case，与后端约定一致）。
        // 密钥类字段留空沿用现有值；uid / nickname 清空时显式删除（全量替换语义）。
        const hasExistingQoderToken =
          Boolean(props.account.credentials_status?.has_access_token) ||
          Boolean(props.account.credentials_status?.has_refresh_token) ||
          Boolean(currentCredentials.access_token || currentCredentials.refresh_token)
        if (!editQoderAccessToken.value.trim() && !hasExistingQoderToken) {
        ...
        Object.assign(
          newCredentials,
          buildQoderCredentials({
            accessToken: editQoderAccessToken.value,
            refreshToken: editQoderRefreshToken.value,
            expiresAt: '',
            deviceToken: '',
            uid: editQoderUid.value,
            realm: editQoderRealm.value,
            baseUrl: newBaseUrl,
            nickname: editQoderNickname.value
          })
        )
    ...
    // TLS 指纹伪装（全平台有效，防封加固）：独立于平台写入 extra，保持账号级 opt-in
    // 语义（关闭时删除键，后端按缺失值回落到默认关闭）。必须放在所有平台专属 extra
    // 块之后，避免被后续块以旧快照覆盖；仅在管理员实际改动时写回，避免用弹窗打开时
    // 的快照覆盖其他平台 extra 中的运行态键。
    {
      const currentEnabled = props.account.enable_tls_fingerprint === true
      const currentProfileId = props.account.tls_fingerprint_profile_id ?? null
      const nextEnabled = tlsFingerprintEnabled.value
      const nextProfileId = tlsFingerprintProfileId.value ?? null
      if (nextEnabled !== currentEnabled || (nextEnabled && nextProfileId !== currentProfileId)) {
        const currentExtra = (updatePayload.extra as Record<string, unknown>) || (props.account.extra as Record<string, unknown>) || {}
        const newExtra: Record<string, unknown> = { ...currentExtra }
        if (nextEnabled) {
          newExtra.enable_tls_fingerprint = true
          if (nextProfileId) {
            newExtra.tls_fingerprint_profile_id = nextProfileId
          } else {
