...
const ApiKeyCreateTestComponent = defineComponent({
  setup() {
    ...
    const toggle = (groupId: number, event: Event) => {
      const checked = (event.target as HTMLInputElement).checked
      formData.group_ids = checked
        ? [...formData.group_ids, groupId]
        : formData.group_ids.filter((id) => id !== groupId)
    }

    return { formData, loading, createdKey, handleCreate, toggle }
  },
  template: `
    <div>
      <form @submit.prevent="handleCreate">
        <input id="name" v-model="formData.name" placeholder="Key 名称" />
        <input id="group-1" type="checkbox" :checked="formData.group_ids.includes(1)" @change="toggle(1, $event)" />
        <input id="group-2" type="checkbox" :checked="formData.group_ids.includes(2)" @change="toggle(2, $event)" />
...
describe('ApiKey 创建流程', () => {
  ...
  it('创建 API Key 调用 API 并显示结果', async () => {
    ...
    expect(wrapper.find('.created-key').text()).toBe('tk-test-key-12345')
  })

  it('选择单个分组后正确传参', async () => {
    mockCreate.mockResolvedValue({
      id: 2,
      key: 'tk-group-key',
      name: 'Group Key',
    ...
    await wrapper.find('#group-1').setValue()
    await wrapper.find('form').trigger('submit')
    await flushPromises()
  ...
  it('选择多个分组时按选择顺序提交 group_ids', async () => {
    mockCreate.mockResolvedValue({
      id: 3,
      key: 'tk-multi-group-key',
      name: 'Multi Group Key',
    })

    const wrapper = mount(ApiKeyCreateTestComponent)

    await wrapper.find('#name').setValue('Multi Group Key')
    await wrapper.find('#group-1').setValue()
    await wrapper.find('#group-2').setValue()
    await wrapper.find('form').trigger('submit')
    await flushPromises()

    // The first entry stays the primary group.
    expect(mockCreate).toHaveBeenCalledWith({
      name: 'Multi Group Key',
      group_ids: [1, 2],
    })
