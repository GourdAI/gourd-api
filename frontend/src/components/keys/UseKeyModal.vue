<template>
  ...
  >
    <div class="space-y-4">
      ...
      <!-- Primary group notice for multi-group keys -->
      <div
        v-if="multiGroup"
        class="flex items-start gap-3 rounded-lg border border-blue-100 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-900/20"
        data-test="multi-group-notice"
      >
        <Icon name="infoCircle" size="md" class="mt-0.5 flex-shrink-0 text-blue-500" />
        <p class="text-sm text-blue-700 dark:text-blue-300">
          {{ t('keys.useKeyModal.primaryGroupNotice') }}
        </p>
      </div>
...
interface Props {
  ...
  /** True when the key binds several groups; only the primary one is shown. */
  multiGroup?: boolean
}
