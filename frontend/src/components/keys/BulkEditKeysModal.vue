    ...
    <form id="bulk-edit-keys-form" class="space-y-5" @submit.prevent="submit">
      ...
      <fieldset :disabled="submitting" class="space-y-5">
        <div class="space-y-2">
          ...
          <div v-if="enabled.group_id" class="space-y-1">
            <MultiGroupSelector
              v-model="groupIds"
              :groups="props.groups"
              :disabled="submitting"
...
const validationError = computed(() => {
  if (enabled.group_id && groupIds.value.length === 0) {
    return t('keys.groupsRequired')
  }
...
const submit = async () => {
  ...
  if (enabled.group_id) updates.group_ids = [...groupIds.value]
