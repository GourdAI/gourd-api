...
describe('UseKeyModal', () => {
  ...
  it('renders Grok Build and OpenCode setup for Grok groups', async () => {
    ...
    expect(allCode).not.toMatch(/^api_key = "tk-grok-test"$/m)
  ...
  it('renders copyable Claude Code setup through the Grok Messages gateway', async () => {
    ...
    expect(codeBlocks.join('\n')).toContain('ANTHROPIC_AUTH_TOKEN="tk-grok-claude-test"')
    ...
    expect(copyToClipboardMock).toHaveBeenCalledWith(
      expect.stringContaining('ANTHROPIC_AUTH_TOKEN="tk-grok-claude-test"'),
      'keys.copied'
    )
  })
  ...
  it('renders Codex custom provider setup through the Grok Responses gateway', async () => {
    ...
    expect(configToml).toMatch(/# experimental_bearer_token = "tk-grok-codex-test"/)
    ...
    expect(codeBlocks.join('\n')).toContain('experimental_bearer_token = "tk-grok-codex-test"')
  })
  ...
  it('offers a downloadable Codex catalog for Composite API keys', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({ Authorization: 'Bearer tk-composite-test' })
      })
    )
