...
const mountModal = () => mount(BulkEditKeysModal, {
  ...
  global: {
    stubs: {
      ...
      // Keep the real child component so the multi-select contract is exercised.
      GroupBadge: true,
      Icon: true,
      PlatformIcon: true
    }
...
describe('BulkEditKeysModal', () => {
  ...
  it('requires at least one group when the group field is enabled', async () => {
    const wrapper = mountModal()
    await wrapper.get('[data-test="enable-group"]').setValue(true)
    await wrapper.get('form').trigger('submit')
    expect(bulkUpdate).not.toHaveBeenCalled()
    expect(wrapper.get('[data-test="submit"]').attributes('disabled')).toBeDefined()
    await wrapper.get('[data-test="group-input-checkbox-7"]').setValue(true)
    await wrapper.get('form').trigger('submit')
    await flushPromises()
    expect(bulkUpdate).toHaveBeenCalledWith([1, 2], { group_ids: [7] })
  })
