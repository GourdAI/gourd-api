...
describe('PlazaModelPricingTable', () => {
  ...
  it('倍率 ≠ 1 时价格列为折后实付价,官方参考以弱化副行同时展示', () => {
    ...
    expect(text).toContain('$3.00')
    expect(text).toContain('$15.00')
    expect(text).toContain('modelPlaza.table.officialTag')
    expect(text).toContain('0.5x')
  })

  it('倍率为 1 且官方价一致时不渲染重复的官方副行', () => {
    const wrapper = mountTable([tokenModel()], 1)
    // 输入/输出列实付与官方一致 → 无官方副行;缓存列官方 1h 价与实付不同 → 有副行
    const cells = wrapper.findAll('tbody td')
    expect(cells[1].text()).toBe('$3.00')
    expect(cells[2].text()).toBe('$15.00')
    expect(cells[3].text()).toContain('modelPlaza.table.officialTag')
  })
  ...
  it('官方价包含 1h 缓存写入价;official_pricing 为 null 时不渲染官方副行', () => {
    const withOfficial = mountTable([tokenModel()], 0.5)
    ...
    expect(withoutOfficial.text()).not.toContain('modelPlaza.table.officialTag')
  })
  ...
  it('per_request 模型按单次价 × 倍率展示,实付区跨三列合并', () => {
    const model = tokenModel({
      name: 'search-tool',
      pricing: {
  ...
  it('token 模型阶梯定价内联进输入/输出列,按倍率折算;档位标签只在输入列', () => {
    const model = tokenModel({
      pricing: {
        billing_mode: 'token',
        input_price: 3e-6,
    ...
    // 输出/缓存列不重复档位标签
    const cells = wrapper.findAll('tbody td')
    expect(cells[1].text()).toContain('≤200K')
    expect(cells[2].text()).not.toContain('200K')
  })
  ...
  it('不再展示平台徽章与平台名,同名模型仅以模型名呈现', () => {
    ...
    // 首列只有模型名,不再附带平台标签
    expect(rows.map((row) => row.find('td').text())).toEqual(['shared-model', 'shared-model'])
    expect(wrapper.text()).not.toContain('Anthropic')
    expect(wrapper.text()).not.toContain('OpenAI')
  })

  it('模型名单元格不再渲染平台徽章(反向断言)', () => {
    const wrapper = mountTable([tokenModel()], 1)
    const firstCell = wrapper.findAll('tbody td')[0]
    // 只剩模型名本身
    expect(firstCell.text()).toBe('claude-sonnet')
    expect(firstCell.find('.rounded-md').exists()).toBe(false)
  })
})
...
describe('PlazaModelPricingTable 长上下文阶梯', () => {
  ...
  it('官方阶梯以副行按档展示且不乘倍率,1h 价内联', () => {
    const wrapper = mountTable([ladderModel()], 0.5)
    const cells = wrapper.findAll('tbody td')
    // 输入列官方副行:官方 ≤272K $5.00 / >272K $10.00
    expect(cells[1].text()).toContain('modelPlaza.table.officialTag')
    expect(cells[1].text()).toContain('$5.00')
    expect(cells[1].text()).toContain('$10.00')
    expect(cells[2].text()).toContain('$30.00')
    expect(cells[2].text()).toContain('$45.00')
    // 缓存列官方副行含写/读两段
    expect(cells[3].text()).toContain('$6.25')
    expect(cells[3].text()).toContain('$12.50')
  ...
  it('无标签的多档按区间生成统一形态(≤上限 / >下限),并按下限升序展示', () => {
    ...
    // 档位标签是每行第一个 span(价格是后续 span,间距由 CSS 提供)
    expect(rows.map((r) => r.find('span').text())).toEqual(['≤100K', '≤200K', '≤1M', '>1M'])
  })
  ...
  it('官方无 intervals 字段(旧响应)时官方列保持平价,实付无阶梯时缓存列保持两行', () => {
    ...
    expect(cells[3].text()).toContain('modelPlaza.table.cacheReadShort')
    expect(cells[3].findAll('.leading-5')).toHaveLength(2)
    expect(cells[3].text()).toContain('(1h')
  })
})
...
describe('PlazaModelPricingTable 分时计价', () => {
  ...
  it('有分时倍率的模型展开为标准行 + 每时段一行,时段行价格按倍率折算且倍率列显示生效倍率', () => {
    ...
    expect(baseCells[4].text()).toContain('0.8x')
    ...
    expect(nightCells[4].text()).toContain('0.4x')
    ...
    expect(peakCells[4].text()).toContain('0.96x')

    // 官方副行不受时段影响
    expect(nightCells[1].text()).toContain('$3.00')
  })
...
describe('PlazaModelPricingTable 窄视口卡片降级', () => {
  beforeEach(() => {
    stubMobileMatchMedia()
  })

  it('窄视口渲染卡片列表而非表格,信息与表格一致', () => {
    const wrapper = mountTable([tokenModel()], 0.5)
    expect(wrapper.find('table').exists()).toBe(false)
    const article = wrapper.find('article')
    expect(article.exists()).toBe(true)
    expect(article.text()).toContain('claude-sonnet')
    expect(article.text()).toContain('$1.50')
    expect(article.text()).toContain('$7.50')
    expect(article.text()).toContain('0.5x')
    // 卡片内价格列带中文列名
    expect(article.text()).toContain('modelPlaza.table.input')
    expect(article.text()).toContain('modelPlaza.table.output')
    expect(article.text()).toContain('modelPlaza.table.cache')
  })

  it('窄视口卡片同样渲染阶梯档位标签与官方副行', () => {
    const model = tokenModel({
      pricing: {
        billing_mode: 'token',
        input_price: 3e-6,
