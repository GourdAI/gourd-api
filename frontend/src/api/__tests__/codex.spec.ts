...
describe('Codex models API', () => {
  ...
  it('fetches a manifest with the current API key without adding it to the catalog', async () => {
    ...
    const result = await fetchCodexModelsManifest('https://example.com/v1', 'tk-user-test')
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: {
          ...
          Authorization: 'Bearer tk-user-test'
        }
      })
    )
    ...
    expect(result.content).not.toContain('tk-user-test')
  })
  ...
  it('rejects a successful response that is not a Codex manifest', async () => {
    ...
    await expect(fetchCodexModelsManifest('https://example.com/v1', 'tk-user-test'))
