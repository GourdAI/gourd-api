...
export type GroupPlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'grok' | 'kimi' | 'zhipu' | 'deepseek' | 'minimax' | 'opencode_go' | 'workbuddy' | 'qoder' | 'trae' | 'composite'
...
export interface ApiKey {
  ...
  /** All bound group IDs; the first entry is the primary group. Always present. */
  group_ids: number[]
...
export interface CreateApiKeyRequest {
  name: string
  group_id?: number | null
  /** Target groups; `group_ids` wins over `group_id` when both are sent. */
  group_ids?: number[]
...
export interface UpdateApiKeyRequest {
  name?: string
  group_id?: number | null
  /** Full replacement for the bound groups; `[]` unbinds every group. */
  group_ids?: number[]
  status?: 'active' | 'inactive'
  ip_whitelist?: string[]
  ip_blacklist?: string[]
...
export type AccountPlatform = 'anthropic' | 'openai' | 'gemini' | 'antigravity' | 'grok' | 'kimi' | 'zhipu' | 'deepseek' | 'minimax' | 'opencode_go' | 'workbuddy' | 'qoder' | 'trae'
