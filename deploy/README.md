...
| **Docker Compose (local source)** | Running your own code changes | Not needed (auto-setup) |
...
| `docker-deploy.sh` | **One-click Docker deployment script (recommended; bundles its deployment templates)** |
| `docker-deploy-local.sh` | One-click deployment that **builds the image from your working tree** (never pulls the published image) |
| `tools/sync-bundled-templates.sh` | Regenerates the deployment templates bundled inside `docker-deploy.sh` |
...
Use the automated deployment script for the easiest setup. The deployment
templates are bundled inside the script, so nothing else is downloaded from
the repository at runtime:

```bash
# Recommended: run inside an empty deployment directory
...
- Writes `docker-compose.yml` and `.env.example` from templates bundled in the script (no downloads)
- Automatically generates secure secrets (JWT_SECRET, TOTP_ENCRYPTION_KEY, POSTGRES_PASSWORD)
- Creates `.env` file with generated secrets
- Creates necessary data directories (data/, postgres_data/, redis_data/)
- Starts the stack and waits for the application health check to pass
...
| `--force` | Regenerate deployment files and secrets (fresh deployments only) |

> Run the script from a separate, empty directory. It refuses to run inside the
> repository's `deploy/` directory, so tracked files are never overwritten.

**After running the script (services are already started):**
```bash
# View logs
...
> Ran with `--no-start`? Start services later with: `docker compose up -d`

### Method 2: Deploy Your Own Code Changes (Build From Source)

`docker-deploy.sh` downloads nothing, but it runs the **published** `weishaw/sub2api`
image from Docker Hub, so it can never contain local modifications. To deploy a
working tree instead, use `docker-deploy-local.sh`, which compiles the checkout it
lives in (frontend + Go backend) into a local image and points Compose at it:

```bash
cd /path/to/sub2api            # your checkout, with your changes
bash deploy/docker-deploy-local.sh
```

**What the script does:**
- Writes `docker-compose.yml` and `.env.example` straight from `deploy/docker-compose.local.yml`
  and `deploy/.env.example` (no bundled copies, so no drift is possible)
- Rewrites the `sub2api` service to `image: sub2api:local` plus a `build:` section pointing at
  the repository root. The tag does not exist in any registry, so a pull can never silently
  replace your code with the upstream image
- Generates the same secrets as `docker-deploy.sh` (JWT_SECRET, TOTP_ENCRYPTION_KEY,
  POSTGRES_PASSWORD) and creates `data/`, `postgres_data/`, `redis_data/`
- Runs `docker compose build` and then `docker compose up -d`, and waits for the health check
- Injects `VERSION` / `COMMIT` build args from `git describe --dirty`, so the running build is
  identifiable in the Admin UI

The deployment files go into a **separate directory** (default `~/sub2api-deploy`), keeping the
checkout clean. Re-running the script after a code change reuses that directory: secrets and
data are preserved and only the image is rebuilt and swapped.
...
| `SUB2API_NPM_REGISTRY` | npm registry for the frontend build (default: `https://registry.npmmirror.com`) |
| `SUB2API_GOPROXY` / `SUB2API_GOSUMDB` | Go module proxy / checksum DB (default: the Dockerfile values) |
| `SUB2API_DEPLOY_HEALTH_TIMEOUT` | Seconds to wait for the health check (default: 900) |

```bash
# Custom deployment directory, custom image tag
cd /path/to/sub2api
SUB2API_LOCAL_IMAGE=sub2api:feature-x bash deploy/docker-deploy-local.sh --dir /srv/sub2api

# Confirm which image the running container uses
cd ~/sub2api-deploy
docker compose ps
docker image inspect sub2api:local --format '{{.Id}} {{.Created}}'
```

> A first source build takes 10-30 minutes and needs a few GB of free disk space; later builds
> reuse the Docker layer cache. Never run `docker compose pull` in this deployment directory -
> this stack is built from source, and `docker compose down -v` / `rm -rf postgres_data/` would
> drop your database.
...
**Recommendation:** Use `docker-compose.local.yml` (deployed by `docker-deploy.sh` and by `docker-deploy-local.sh`) for easier data management and migration.
...
> The one-click script (`docker-deploy.sh`) saves the local-directory compose file
> as `docker-compose.yml`, so its commands omit `-f` (for example
> `docker compose up -d`). The examples below target a manual deployment that
> keeps the `docker-compose.local.yml` filename.
>
> For a **local-source** deployment (`docker-deploy-local.sh`) the same commands
> apply from the deployment directory, except that updates never use `pull`: rebuild
> with `docker compose build sub2api && docker compose up -d`, or simply re-run
> `bash deploy/docker-deploy-local.sh` after changing the code.
...
### Maintaining the Bundled Deployment Templates

`docker-deploy.sh` embeds byte-identical copies of `docker-compose.local.yml`
and `.env.example`. Whenever either canonical file changes, regenerate the
bundled copies before committing:
...
from the canonical files, so CI catches a forgotten sync.

`docker-deploy-local.sh` reads `docker-compose.local.yml` and `.env.example`
directly at run time, so it has no bundled copies to maintain and needs no sync
step. `deploy/tests/docker-deploy-local-test.sh` verifies that the compose file
it generates keeps the canonical template intact outside the `sub2api` image
block, and that no published image reference survives.
